...
**`delegateServers` constraint (KPR-184):** the 10 KPR-122-ported MCPs, plus later in-process servers (`memory`, `structured-memory`, `event-bus`, `callback`, `contacts`, `schedule`, `team`, `admin`, `code-search`, `workflow`, `worker-pool` — KPR-390), cannot appear in `delegateServers`. They're in-process post-KPR-122 and the SDK's `AgentDefinition.mcpServers` type doesn't accept in-process configs. Use `coreServers` instead. The admin tool (`agent_create` / `agent_update`) rejects malformed inputs; the registry sanitizes pre-existing data at load time and logs an error so the operator can clean up via `admin_agent_update`. Constant lives at `src/agents/in-process-servers.ts`.
...
**Grok routes through a self-hosted CLIProxyAPI gateway (KPR-384; grok itself is a native Lane B adapter since KPR-392).** xAI's own Anthropic-compat `/v1/messages` rejects any tool `input_schema` lacking the `required` array (spec violation — valid JSON Schema, accepted by Anthropic; 13 of the CLI's builtin/plugin tool schemas omit it), so a direct `api.x.ai` spawn 400s on every realistic toolkit — that history is why the gateway exists, though `GrokGatewayAdapter`'s own chat-completions call shape (hive-built, not the Claude CLI's) never exercises that validator; the mitigation, if the same quirk ever resurfaces on a schema hive itself builds, is the one-line `parameters.required ??= []` edge-2 note in the adapter, not a gateway change. In practice `tool-bridge.ts:538` already fills `required: []` on every bridged schema, so that quirk is structurally impossible on the Lane B path today. `GrokGatewayAdapter` therefore pins `baseUrl` to an **operator-hosted, loopback-only CLIProxyAPI gateway** (default `http://127.0.0.1:8317`; non-secret `GROK_GATEWAY_URL` override, resolved per spawn and validated — https, or http to loopback only; version-pinned — the fleet runs `io.keepur.grok-gateway` at v7.2.134) that fronts the vendor's OpenAI-format backend. The credential is the ordinary env-key chain: `GROK_GATEWAY_KEY` (an entry from the gateway config's `api-keys` allowlist, env→Honeypot per spawn via the manager's `resolveGrokModuleSlice`, `hive credentials add GROK_GATEWAY_KEY`). The **gateway** owns the `grok login` subscription OAuth session and its refresh lifecycle; hive no longer reads or writes `~/.grok/auth.json` — KPR-371's `grok-oauth.ts` machinery stays deleted, and `PassthroughCredential` (`passthrough-providers.ts`) is env-key-only again with no grok row at all (the `kind` discriminant remains for future re-widening). This is the one sanctioned exception to the "no translation proxies" canon, carried forward from Lane A onto Lane B: the operator owns the middlebox. Subscription exposes only `grok-4.6`/`grok-4.5`; unlike the retired Lane A clamp, grok's native `xhigh` effort **is expressible** now (`reasoning_effort` delivers verbatim, `minimal`/`none` coerce to `low`). Sessions are `stateless-replay` on `provider_turn_history` (like codex, minus encrypted reasoning — grok has none — and minus codex's 4xx self-heal, since a 4xx on hive-composed replay items is a real bug, not a backend quirk). Ops surfaces key on the route: a Kimi outage trips the `kimi` breaker/outage queue only; a grok outage trips the `grok` breaker/outage queue only, same as any other provider. `costUsd` reports 0 with real token counts (the Lane A nominal-cost caveat retired with the Lane A move). Rollback = set the agent's `model` back + SIGUSR1, unchanged.
