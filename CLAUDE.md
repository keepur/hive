...
**Lane A passthrough (KPR-346, KPR-371):** `kimi/…`, `deepseek/…`, and `grok/…` route through the full Claude runtime (`ClaudeAgentAdapter`/`AgentRunner` — MCP tools, skills, hooks, memory, resume) with per-spawn env substitution: vendor Anthropic-compat base URL + Honeypot-resolved key (`KIMI_API_KEY`/`DEEPSEEK_API_KEY`, per-spawn env→Keychain — `hive credentials add` takes effect next spawn), foreign-model pins incl. subagents, `ENABLE_TOOL_SEARCH` forced off. Table: `src/agents/provider-adapters/passthrough-providers.ts` (defaults `kimi-k3` / `deepseek-v4-pro` / `grok-4.6`, overridable via `KIMI_AGENT_MODEL`/`DEEPSEEK_AGENT_MODEL`/`GROK_AGENT_MODEL`). Missing credential ⇒ `TurnAssemblyError` (breaker-invisible).

**Grok's credential is the one structural exception (KPR-371).** The table's `credential` field is discriminated: kimi/deepseek declare `{kind:"env-key"}` (env→Keychain, unchanged); grok declares `{kind:"oauth-file", path:"~/.grok/auth.json"}` — the **subscription OAuth credential the `grok` CLI owns**. There is no `XAI_API_KEY` path and **no `hive credentials` entry** (nothing to paste); the operator runs `grok login` once per machine. Hive **reads and writes** that file: the access token lasts 6h and the refresh token **rotates on use**, so `src/agents/provider-adapters/grok-oauth.ts` performs a public-client OIDC refresh (token endpoint discovered from `oidc_issuer`, never hardcoded) under a **per-path single-flight guard** and atomically writes the rotated pair back at 0600, preserving unknown fields. Two deliberate non-failures: a grant rejected because another process refreshed first re-reads and proceeds; a transient `auth.x.ai` fault with a still-valid on-disk token warns and proceeds. Only a genuinely revoked token surfaces `grok login`. Mid-turn expiry is accepted and self-healing (the 60-min refresh threshold covers any turn under an hour; longer ones ride the breaker + outage queue). Subscription exposes only `grok-4.6`/`grok-4.5`, and grok's native `xhigh` effort is **not expressible** (the Lane A clamp drops out-of-set values). Ops surfaces key on the route: a Kimi outage trips the `kimi` breaker/outage queue only. Caveats: no Anthropic server-side tools (WebFetch/WebSearch), `costUsd` nominal (Claude pricing), sessions are `client-transcript` (resume replays against a cold vendor cache). Rollback = set the agent's `model` back + SIGUSR1.
