...
// Watermark state lives in local hive DB, NOT the source DB — the source may be
// remote Atlas production (read-only role) or a disposable local copy.
const EMBED_META_URI = process.env.EMBED_META_URI ?? "mongodb://localhost:27017/hive_dodi";
...
async function upsertPoints(
  ...
  for (let i = 0; i < points.length; i += QDRANT_UPSERT_BATCH) {
    ...
    // Transient connection resets happen on large payloads — retry like embedBatch does
    for (let attempt = 1; attempt <= OLLAMA_MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt >= OLLAMA_MAX_RETRIES) throw err;
        const delay = OLLAMA_RETRY_DELAY_MS * attempt;
        console.warn(`  Qdrant upsert attempt ${attempt} failed (${collection}): ${err.message} — retrying in ${delay}ms`);
        await sleep(delay);
      }
    }
  }
...
async function processObjectType(
  ...
  const metaCol = metaDb.collection("embed_meta");
  ...
  const cursor = sourceCol.find(query, { projection }).sort({ [watermarkField]: 1 }).allowDiskUse(true);
  let embedded = 0;
  let skipped = 0;
...
async function main(): Promise<void> {
  ...
  // Connect to MongoDB (source may be remote Atlas production)
  const mongo = new MongoClient(MONGODB_DODI_URI);
  await mongo.connect();
  const db = mongo.db();
  console.log("  MongoDB:   connected");

  // Separate connection for watermark state (always local)
  const metaMongo = new MongoClient(EMBED_META_URI);
  await metaMongo.connect();
  const metaDb = metaMongo.db();
  ...
  for (const config of configs) {
    results[config.objectType] = await processObjectType(db, metaDb, qdrant, config, projectMap);
  }

  // Design iterations — latest version per design (separate processing)
  if (!objectsFilter || objectsFilter === "design_iteration") {
    results["design_iteration"] = await processDesignIterations(db, metaDb, qdrant);
  }

  await mongo.close();
  await metaMongo.close();
