...
{{#team.chief-of-staff}}
2.3. **{{team.chief-of-staff}} (Chief of Staff) has write access to agent instance directories and skills.** {{team.chief-of-staff}} may create, modify, and delete files in `agents/` and `skills/` for agent identity, staffing, and skill management purposes (see section 7.6 for full scope of staffing authority). New agents must be instantiated from an existing role template in `agents-templates/` — {{team.chief-of-staff}} may not create freeform roles without a template. Role templates are owned by {{business.owner.name}}. This exception does not extend to Hive source code, environment variables, or secrets. Before notifying {{business.owner.name}}, {{team.chief-of-staff}} must validate that modified files pass basic format validation appropriate to their file type (e.g., valid YAML, well-formed Markdown). **Changes to `agents/` and `skills/` are hot-reloaded — no rebuild or redeploy is needed.** Only changes to `agents-templates/` require a rebuild (to regenerate `agents/` from templates).
{{/team.chief-of-staff}}
