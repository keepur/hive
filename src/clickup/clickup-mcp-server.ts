...
server.registerTool("clickup_list_workspaces", {
...
server.registerTool("clickup_list_spaces", {
  title: "List Spaces",
...
server.registerTool("clickup_list_folders", {
  title: "List Folders",
...
server.registerTool("clickup_list_lists", {
  title: "List Lists",
...
server.registerTool("clickup_list_folderless_lists", {
...
server.registerTool("clickup_search_tasks", {
  ...
  try {
    ...
    const lines = tasks.map(formatTask);
    return { content: [{ type: "text", text: `Search results for "${query}" (${tasks.length}):\n\n${lines.join("\n\n")}` }] };
  } catch (e: any) {
    return { content: [{ type: "text", text: e.message }], isError: true };
  }
});
...
server.registerTool("clickup_get_task", {
  ...
  try {
    ...
    const customFields = t.custom_fields?.filter((f: any) => f.value != null).length
      ? `\n  Custom fields:\n${t.custom_fields.filter((f: any) => f.value != null).map((f: any) => `    ${f.name}: ${JSON.stringify(f.value)}`).join("\n")}`
      : "";
