...
describe("Lane B adapter error strings → ProviderFaultKind (KPR-391 §8 cross-check)", () => {
  ...
  it.each([
    ...
    // grok-adapter.ts — grokErrorMessage C5 decoration at a 429 status.
    "Grok request failed (429): too many requests",
  ])("rate-limit: %s", (s) => expect(faultKind(s)).toBe("rate-limit"));

  it.each([
    // grok-adapter.ts — grokErrorMessage C5 decoration at a 503 status.
    "Grok request failed (503): upstream unavailable",
  ])("server-error: %s", (s) => expect(faultKind(s)).toBe("server-error"));

  it.each([
    // grok-adapter.ts — consumeGrokSse edge-3 drop decoration: a stream that
    // ended without finish_reason (not aborted by hive) is a drop, phrased
    // to land on the connect-fail row via "terminated". Deliberate
    // attribution: xAI's endpoint is grok route infrastructure — its death
    // classifies as a grok provider fault by design (KPR-306/307 key on the
    // route, not on any particular vendor detail). assembleToolCalls' own
    // incomplete-fragment message is a different string that also carries
    // "terminated" and is pinned separately below. Raw `fetch
    // failed`/`ECONNREFUSED` throws from the adapter's own fetch call are
    // already row-pinned by the shared FAULT_PATTERNS connect-fail regex and
    // need no adapter-specific row here.
    "Grok stream ended without finish_reason — connection terminated mid-stream",
    // grok-adapter.ts — Edge 3 spirit guard: finish_reason=tool_calls with
    // zero assembled tool calls is a stream-shape fault, phrased onto the
    // same connect-fail row via "terminated" (r1 fix — was previously worded
    // "malformed stream", which matched no FAULT_PATTERNS row and
    // misclassified non-provider).
    "Grok stream signaled tool_calls but no tool calls were assembled — connection terminated mid-stream",
    // grok-adapter.ts — assembleToolCalls incomplete-fragment guard (id/name
    // never arrived for an indexed fragment): same connect-fail routing via
    // "terminated".
    "Grok stream delivered an incomplete tool_call at index 0 (missing id or name) — connection terminated mid-stream",
