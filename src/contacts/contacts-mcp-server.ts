...
server.registerTool("contacts_search", {
  ...
  description: "Search contacts by name, phone number, email, or tag. Returns matching contacts with all available details.",
  inputSchema: {
  ...
  try {
    ...
    if (digits.length >= 7) {
      ...
      const phoneResults = await contacts
        .find({ "phones.number": e164 })
        .limit(limit)
        .toArray();
    ...
    if (query.includes("@")) {
      const emailResults = await contacts
        .find({ email: query.toLowerCase() })
        .limit(limit)
        .toArray();
    ...
    // Tag search
    const tagResults = await contacts
...
server.registerTool("contacts_list", {
  ...
  try {
    ...
    return { content: [{ type: "text", text: `${total} contacts${tag ? ` tagged "${tag}"` : ""}${source ? ` from ${source}` : ""}:\n\n${lines.join("\n")}` }] };
  } catch (e: any) {
    return { content: [{ type: "text", text: e.message }], isError: true };
  }
});
