...
server.registerTool(
  "quo_phone_numbers",
  {
    ...
    description:
      "List all Quo phone numbers in the workspace. Use this to find phone number IDs needed for other operations.",
    inputSchema: {},
  },
  async () => {
    try {
...
server.registerTool(
  "quo_send_sms",
  {
    title: "Send SMS",
    description: (() => {
      const lineDescs = Object.entries(LINES)
        .map(([k, v]) => `"${k}" for ${v.label} ${v.number}`)
        .join(", ");
    ...
    inputSchema: {
      ...
      from: z
        .string()
        .optional()
        .describe('Line to send from: "main", "personal", or a PNxxx ID. Defaults to personal.'),
    },
  },
  async ({ to, content, from }) => {
    try {
      ...
      if (!fromId) {
        return {
          content: [{ type: "text", text: "No sending phone number configured. Use quo_phone_numbers to find one." }],
          isError: true,
        };
      }
  ...
  "quo_list_messages",
  {
    ...
    inputSchema: {
      line: z
        ...
        .optional()
        .default("personal")
        .describe('Which Quo line: "main", "personal", or PNxxx ID. Defaults to personal.'),
  ...
  "quo_list_conversations",
  {
    ...
    inputSchema: {
      line: z
        .string()
        .optional()
        .describe('Filter by line: "main", "personal", or PNxxx ID. Shows all lines if omitted.'),
      maxResults: z.number().optional().default(20).describe("Max results (1-100, default 20)"),
...
server.registerTool(
  "quo_lookup_contact",
  {
    ...
    description:
      "Look up a contact by phone number. Returns the contact's name, company, email, and other details if found.",
    inputSchema: {
    ...
    try {
      ...
      for (let page = 0; page < 20 && !found; page++) {
        ...
        for (const contact of data.data ?? []) {
          const phones: string[] = (contact.defaultFields?.phoneNumbers ?? []).map(
            (p: any) => p.value?.replace(/\D/g, "") ?? "",
          );
