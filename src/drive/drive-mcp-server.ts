...
const GWS = process.env.GWS_PATH || (() => {
  try { return execSync("which gws", { encoding: "utf-8" }).trim(); }
  catch { return "gws"; }
})();
...
server.registerTool("drive_upload", {
  ...
  if (!SHARED_FOLDER) {
    return { content: [{ type: "text", text: "Drive shared folder not configured (DRIVE_SHARED_FOLDER)." }], isError: true };
  }
  ...
  try {
    ...
    // Get shareable link
    const fields = JSON.stringify({ fileId: data.id, fields: "id,name,webViewLink,webContentLink", supportsAllDrives: true });
...
server.registerTool("drive_list", {
  ...
  description: "List files in the company shared Drive folder. Useful to see what reports and documents have been shared.",
  inputSchema: {
