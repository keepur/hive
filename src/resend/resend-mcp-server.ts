...
server.registerTool(
  "send_email",
  {
    title: "Send Email",
    description:
      `Send an email via Resend. Emails are sent from ${FROM_ADDRESS}. ` +
    ...
    inputSchema: {
      ...
      html: z
        .string()
        .optional()
        .describe("Email body (HTML) — if provided, used instead of plain text for rich formatting"),
      reply_to: z
        .string()
        .optional()
        .describe("Reply-To address (e.g. the assigned rep's email). Customer replies go here."),
      cc: z
        .union([z.string(), z.array(z.string())])
        .optional()
        .describe(`Additional CC addresses. ${DEFAULT_CC} is always included automatically.`),
      attachments: z
        .array(
          z.object({
            ...
            filename: z
              .string()
              .optional()
              .describe("Override the filename in the email (defaults to the file's basename)"),
          }),
        )
        .optional()
        .describe("Files to attach to the email. Use the local file path from Slack file downloads."),
    },
  },
