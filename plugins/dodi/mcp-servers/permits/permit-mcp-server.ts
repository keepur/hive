...
server.registerTool("permit_search", {
  ...
  const lines = docs.map((d) => {
    const parts = [
      ...
      ...(d.ownerName ? [`  Owner: ${d.ownerName}${d.ownerPhone ? ` | ${d.ownerPhone}` : ""}${d.ownerEmail ? ` | ${d.ownerEmail}` : ""}`] : []),
      ...(d.assessedValue ? [`  Property: $${d.assessedValue.toLocaleString()} assessed | ${d.sqft || "?"} sqft | Built ${d.yearBuilt || "?"}`] : []),
    ];
    return parts.join("\n");
  });
...
server.registerTool("permit_stats", {
...
server.registerTool("permit_contractors", {
  ...
  const lines = docs.map((d) => {
    const parts = [
      ...
      ...(d.cslbLicenseNo ? [`  CSLB: #${d.cslbLicenseNo} (${d.cslbStatus || "unknown"}) — ${d.cslbClassifications || ""}`] : []),
...
server.registerTool("permit_export_csv", {
  ...
  const headers = [
    "City", "Address", "Filed Date", "Permit Number", "Description",
    "Valuation", "Tier", "AI Score", "AI Classification",
    "Contractor", "Applicant",
    "Owner Name", "Owner Phone", "Owner Email",
    "Assessed Value", "Year Built", "Sqft",
  ];
  ...
  const rows = docs.map((d) => [
    d.city, d.address, d.filedDate, d.permitNumber, d.description,
    d.valuation, d.tier, d.aiScore, d.aiClassification,
    d.contractor, d.applicant,
    d.ownerName, d.ownerPhone, d.ownerEmail,
    d.assessedValue, d.yearBuilt, d.sqft,
  ].map(escCsv).join(","));
