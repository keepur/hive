...
- **Superseded in part — KPR-431 (2026-09):** rules (a) and (b) above no longer hold for the **system-sender** and **router-catch** paths, which now deliver the static-tier `resourceLimits` like the router-on human path (the agent-def `maxTurns`/`budgetUsd` are dead config on every Claude/router-on turn; `timeoutMs` stays live via KPR-422). The router-off (c), voice, and pilot/Lane A (d) paths keep `resourceLimits: undefined` exactly as written here. The §4.2 system-sender path-preservation pin was rewritten with citation, not weakened; the KPR-338 plan's NV-c is inverted by KPR-431's own negative-verify (delivering static limits on the system path is now the pinned behaviour). Spec: `keepur/hive-docs` `internal/specs/2026-09-04-kpr-431-system-sender-resource-limits-design.md`.
