...
- **Sequencing inside the ticket (compile-green per commit):** manager first, router second. Task 1 restructures `agent-manager.ts` against the *old* `routeModel` contract (result's tier/model/limits/provider simply ignored) — **fixed-tier behavior is final after Task 1; effort emission converges to its final shape at Task 2** (suppression-only delta, safe: the old router emits no effort on H2/H3 hits and drops post-cap-haiku effort, so interim turns that will get `effort: "low"` after Task 2 get none at Task 1 — never the reverse). Task 2 then shrinks the router contract and updates the one call site. This avoids a single mega-commit spanning both files and both test suites; the cost is a small deliberate double-touch in `agent-manager.test.ts` (the `makeRouterResult` helper and the `routeModel` arg pins are updated again in Task 2 — flagged inline).
...
- **KPR-306 breaker describe — assertion meanings unchanged, harness touched in exactly two scoped places:** the "three consecutive hard faults…" test's router-enabled fast-fail turn and count-delta pin retarget to `agent-s` (T1h — post-haiku-skip, `agent-a` never calls `routeModel`, so the delta pin would pass vacuously and "breaker permit before any model-router spend" would be exercised nowhere), and its raw legacy-shape `routeModel` stub reshapes to `makeRouterResult()` (T2d). Every other test in the describe, and all KPR-313 guard/persist and KPR-220 coordinator describes: **untouched and green** (any failure = leakage across R3/R7/313 surfaces).
...
- `agent-manager.test.ts` — KPR-307 providerFor/timedOut describes, KPR-313 session-identity + persist + churn-mint describes, KPR-220 coordinator/reflection describes: **zero assertion changes**. KPR-306 breaker-wrap describe: **assertion meanings unchanged; harness stubs reshaped per T2d** and the fast-fail router-spend pin retargeted to `agent-s` per T1h — no other edits inside the describe. KPR-311/312 seam describes: assertion meanings preserved except where tier-selection was itself the asserted behavior — those are removed/rewritten **with spec citation in a comment**, never silently weakened.
...
**Goal:** after this task the engine's *fixed-tier* behavior is final — no turn ever runs a model other than `agentConfig.model`, haiku/effort-incapable agents skip the classifier, limits are static-tier-sourced and path-preserving, telemetry/audit read static facts. Effort emission converges to its final shape at Task 2 (suppression-only interim delta — the old router emits no effort on H2/H3 hits and drops post-cap-haiku effort, so some turns that will get `effort: "low"` after Task 2 get none yet; never the reverse). The router still has its old signature (Task 2 shrinks it); its tier/model/limits/provider outputs are simply no longer read.
...
- **`provider circuit breaker at the wrap point (KPR-306)` describe — harness-only retarget, assertion meanings unchanged:** in "three consecutive hard faults open the breaker; the next spawnTurn fast-fails before the adapter", retarget the router-enabled portion to `agent-s`: the fast-fail turn becomes `smsCtx({ agentId: "agent-s", threadId: "sms:line-1:fast-fail" })` (the helper accepts an `agentId` override; the three `tripBreaker` setup turns stay on `agent-a` — the breaker is per-provider, both are claude). Post-haiku-skip, `agent-a` never calls `routeModel`, so the existing count-delta pin (test comment at the `routerCallsBefore` capture) would pass vacuously; on `agent-s` (sonnet-class, still router-eligible) it stays live — an admitted turn WOULD spend a router call, so the zero-delta across the fast-failed turn genuinely pins the CLAUDE.md invariant "breaker permit before any model-router spend". Refresh that test's router-enabled comment block to say why `agent-s` is now required (haiku-static agents skip the router entirely, KPR-338). Scope: this stub/ctx/comment reshape is the ONLY edit inside the 306 describe — the breaker-open, fast-fail-before-adapter, and cleanliness assertions are byte-identical. (Its legacy-shape `routeModel` stub is reshaped to `makeRouterResult()` in T2d, not here — at T1 the old result type still accepts it.)
- **Negative-verifies NV-a, NV-b, NV-c, NV-d** (Testing Contract): run now, record evidence, restore.
- (NIT-4 awareness: `import type { AgentProviderId }` in `agent-manager.test.ts` remains used by the `smsCtx` helper's `sessionProvider` field — verified at test file `:826` — so it survives the clamp-test deletion; if typecheck ever flags it unused after these edits, delete it then.)

**Verify:**
```bash
npx vitest run src/agents/model-router.test.ts src/agents/agent-manager.test.ts   # green
npx vitest run src/agents/                                                        # green (306 harness-retargeted per 1h; 307/313/220 untouched)
...
```
- Every `mockResolvedValue*(...)` router stub drops tier/model/limits/provider keys (they are now excess-property type errors — the compiler enumerates the sites). **This includes one site inside the KPR-306 breaker describe** — the raw legacy-shape stub in "three consecutive hard faults…" (the `{ tier: "sonnet", model: "claude-sonnet-4-7", … resourceLimits: {…} }` literal) becomes `makeRouterResult()`. That is a harness-only reshape; the test's assertions are untouched (its `agent-s` retarget already happened in T1h) — this named authorization is what keeps the Regression Surface's "assertion meanings unchanged" claim and this compiler-driven sweep consistent.
...
Post the two JSON blobs per instance, plus a 3–5 sentence reading (downgrade frequency; whether downgraded turns skew cache-cold — i.e. whether the "saving" was cache-negative), as a Linear comment on KPR-338 titled **"§3.5 validation rider evidence"**. **The evidence comment must carry this caveat verbatim:** the join buckets historical telemetry against `agent_definitions.model` *as of query time* — any agent whose static model changed since a row was written is mis-bucketed, so treat the numbers as directional, not exact. This feeds the §3.4 effort-classifier demotion decision later; it changes nothing in this plan regardless of outcome.
...
6. **Interim state between Tasks 1 and 2 is safe (fixed-tier final; effort converging):** fixed-tier behavior is final at Task 1; effort emission reaches its final shape only at Task 2 — the interim delta is suppression-only (the legacy router emits no effort on H2/H3 hits and drops post-cap-haiku effort, so turns that will get `effort: "low"` post-T2 get none at T1, and Task 1's merge can never deliver effort to an effort-rejecting model).
