...
| --- | --- | --- |
| `ServiceController.bootout` in `src/deployment/services.ts` (captures `children(pid)` before bootout and waits for every captured descendant) | descendant wait is withdrawn as a stop gate, including the `service process tree survived bootout` failure | keep `markIrreversible`, bootout, loaded-state and deadline behavior; the worker stop condition becomes supervisor PID/start-time exit plus release of its recorded health listener (engine: PID exit), with an unexpected listener owner reported and never killed; remove the `service process tree survived bootout` error and update the `services.test.ts` cases that asserted it or descendant survival |
...
- Round 3 (`plan-review/2/frontier`): registry-barrier `supervisor-exited` reconciliation now gates on supervisor PID/start-time exit plus health-listener release (foreign owner reported, never killed), with no tree/descendant absence; impact table names `service process tree survived bootout` as withdrawn; aligned old-boot mailbox cleanup, Task 11 Step 2 T10 skip scope, the chunk 5 self-test placement and tarball-path-only archive-member hashing.
