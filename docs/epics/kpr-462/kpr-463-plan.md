...
- Unit/integration expected: Vitest exit 0, no skipped contract cases. The only permitted skips are the real-`sandbox-exec` cases (T2, T7 detached-writer, T10) on a non-macOS host, each reporting an explicit reason; such a skip never counts toward the deployment proof, and the macOS ARM64 CI job must run them unskipped.
...
| `src/cli/prereqs.ts`, `src/cli/update-preflight.ts` + tests | init/resume/update/first-adoption confinement and clone prerequisites as a fail-closed self-test; never consulted by ordinary rollback |
...
| S11 | S10 | Task 11 Steps 1–4 CI/release wiring, full contract, fresh review and final clean candidate. | All Task 11 gates, including Node22 and macOS ARM64/Node24 (unskipped T2/T7-detached-writer and a T10 engineering run), plus exact clean archive identity. |
