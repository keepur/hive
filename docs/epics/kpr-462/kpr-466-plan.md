...
**Interfaces, compatibility, and invariants:** Standing Invariants 1–4. L2 scoring specifically: do **not** copy KPR-464's frozen "no optional opening" cell (spec §4.2, canon R6) — L2 finalizes in two stages. **Stage 1 (JSONL-only):** check the defect condition in the classification table below; a match is `fail` on its own, regardless of May's report — the one case the classification table alone finalizes L2. **Stage 2 (only when Stage 1 does not match):** classify the ordering into one of the four pass-track/`unknown` classes — each written to exclude the Stage-1 defect condition by its own wording — then finalize strictly from the corroboration table that follows. A Stage-2 class's result is never finalized from the classification table alone, and a Stage-1 `fail` is never re-derived from, or softened by, the corroboration table, which carries no row for it.

**Acceptance criteria:**

**Call 2 — L2.** May says hello immediately before or at answer (itself controlled script text). Classify from the JSONL fields above in two stages, Stage 1 always first — both classification steps alone may be done from JSONL:

**Stage 1 — JSONL-only defect check, checked before any Stage-2 classification:**

| Condition | Result |
| --- | --- |
| Suppression was required (final or accept before request) but an opening was still requested; or a late-final opening remained queued behind the replacement instead of being cleared before it proceeded; or the replacement required another utterance before it was accepted; or a later `origin: "opening"` speech replayed after a replacement already progressed | `fail` |

If this row's condition holds, record L2 `fail` immediately — do not classify into a Stage-2 class and do not consult the corroboration table below. This is the one case a table alone finalizes L2 (spec §3.1: "machine evidence shows a defect on a call the caller also experienced").
...
**L2 corroboration mapping (executable — finalizes L2's result for the four Stage-2 classes; a Stage-1 defect is already `fail` before this table is ever consulted):**
...
- [ ] Place Call 2 (Mokie `voice_call` preferred); say hello immediately before/at answer; check the Stage 1 defect condition first from content-free JSONL fields — if it matches, record L2 `fail` and stop; otherwise classify the ordering into a Stage-2 class per the table above; record May's sanitized first-audible class and her stale-replay confirmation; finalize the result — `pass`, `fail`, or `unknown` — strictly from the L2 corroboration mapping table above; never record `pass` from JSONL ordering alone, never skip the Stage-1 defect check on the way to a Stage-2 class, and never downgrade a mapped `fail` (a Stage-1 defect match, stale replay reported on a Stage-2 class, or `heard_opening`/`heard_both` under `l2_suppressed`) to `unknown`; retry once if voicemail/no-answer.
