...
No await or foreign callback may intervene between that final guard and `runOnce`. Each attempt retains its own signal; do not reset or replace the failed attempt's signal before the decision, reconstruct it from current manager state, or serialize it into trace/response/store payloads. Terminal diagnostics consume only the derived bounded stopped/error classification. Returned provider/session errors without request cancellation or a typed stopped cause remain eligible for the existing one retry, including when the watchdog sets `result.aborted`/`timedOut` or their lease signal is aborted for an ordinary failure. In the returned-error branch, `result.aborted` is not evidence of request cancellation: retain the existing error-first precedence and original error reason. The later explicit `result.aborted` classification applies only after the error branch has been ruled out; successful result accounting and that no-error abort classification remain unchanged.

Request outcome categories: `completed` for successful normal response including no-content; `cancelled` for local disconnect/request-canceled exception or explicit returned abort without returned errors; `failed` for lookup/admission/provider/SSE failure; `incomplete` only when execution finalization itself cannot observe an end (reader reconstructs process loss). Keep classified `errorClass` separate. Do not classify disconnect as barge-in. No-content completion carries `firstText: { value: null, reason: "not_reached" }` and `generatedAudio: unknown`.
...
Add a real loopback HTTP watchdog regression using the production warm demux/watchdog and manager/dispatcher/adapter with a controlled Query: on a live resumed request before text, fire the watchdog, reject `Query.interrupt()`, then settle its pending read as EOF and as rejection. Assert the first result retains its original error, `aborted: true`, `timedOut: true` and observed usage while the request signal is live and the lifetime cause is not `AgentStoppedError`; it remains a failed attempt, not request cancellation. Each variant receives exactly one fresh/full-transcript outer retry, emits normal replacement text/done, and records one request terminal plus two attempt terminals with the same bridge turn ID. Repeat with prior response bytes, actual request cancellation, and typed stop (including immediate restart): each prohibits the retry, retains its appropriate failure/cancellation/stopped classification, and emits no replacement output. For typed-stop controls, hold the interrupt promise and stop the still-active lifetime before rejecting it; a later stop does not reclassify an ordinary failure already closed/released. Also preserve ordinary pre-text provider/session-error retry and no-error `aborted` classification controls. These product HTTP cases are required even when test-local extracted snippets have passed.
