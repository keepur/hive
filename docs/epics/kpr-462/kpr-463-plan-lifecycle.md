...
| `children(pid)` | `ps -axo pid=,ppid=,lstart=,command=` captured privately | observation-only source for chunk 4's `process-census` pilot inventory check; never consulted by stop, rotation, staging settlement or health decisions (remove it if that check does not use it) |
...
| `bootout(def)` | `launchctl bootout gui/<uid>/<label>` | only after quiescence for planned operations; mark shutdown irreversible before invocation; verify the label is unloaded and the captured supervisor PID/start time has exited; for the worker also verify its recorded health listener is released (no owner) |
...
After worker bootout, wait at most 30 seconds for the captured supervisor PID/start-time tuple to exit and for its recorded health listener to have no owner. Verified quiescence already established that the supervisor had no live SDK jobs; a straggling SDK helper owns no instance listener and cannot touch the verified clone, so no descendant census is taken and no descendant gates rotation (spec §5.1 step 3). A PID reused by another executable is not an owned process and must not be signaled. A listener still owned by the old supervisor keeps waiting within the budget; a listener owned by any other process is recorded as an unexpected owner and fails the stop (never killed). Only after the worker supervisor has exited and released its listener may engine bootout occur, with the same 30-second exit budget verifying the engine PID/start-time exit. A bounded stop failure enters recovery; do not escalate to broad `kill -9` or port killing. Automatic recovery can stop positively identified failed-candidate supervisors (launchd bootout, then the same exit and listener checks), but cannot claim rollback success while a candidate supervisor remains live or a required instance listener has an unexpected owner.
...
Require XML special characters, absolute paths, alternate config, symlink containment, enabled/disabled/previously installed transitions, all start/stop orders, owner mismatch, a surviving non-listener SDK helper that does not block worker stop, a listener held by the old supervisor past the budget (stop failure) or by a foreign process (unexpected owner, never killed), missing entrypoint before write, repeated no-op behavior and second-instance exclusion.
...
The original lifecycle-only record shape below is the preserved schema-1 starting point. Remaining S7 migrates producers/consumers to chunk 5’s strict schema-2 work union; its prior/profile/artifact fields belong only to lifecycle work, and every staging work kind also records its job directories, self-test outcome, promotion method and clone-verified state as chunk 5 Task 8 Step 1a.3 specifies. Bootstrap/registry reconciliation dispatches before reading them. Incomplete historical schema-1 records remain unresolved.
...
Dry-run branches **before** lock/directory creation, freeze, npm lookup or config-secret import. It reads only config paths/manifests/current marker as needed and prints target, phase list, resolved selectors, voice enablement, known recovery profile and unknown evidence still needed. It performs no fetch/install/writes/vendor probes/service control/notifications, and runs no confinement self-test, promotion-method probe or artifact job (those write); it may report whether `/usr/bin/sandbox-exec` is present as unverified prerequisite information. Test filesystem snapshots and all injected side-effect spies remain identical/zero.

- [ ] **Step 3:** Implement `artifact.ts` operations with these exact boundaries:

| Operation | Inputs and code path | Required checks |
| --- | --- | --- |
| confinement preflight | chunk 5 Task 8 Step 1a.3 self-test and promotion-method selection, before the first artifact job; skipped entirely by ordinary rollback, stop, start and restart | missing `/usr/bin/sandbox-exec` or a self-test that does not deny fails closed before any staging or service change; method (clone or full copy with free-space gate) recorded in the marker and fixed for the operation |
| resolve tag | one tag/version normalized by removing a leading `v`; confined job running `npm pack @keepur/hive@<selected> --json --pack-destination <fetch-job-dir>` | array/object JSON parse; exactly one produced local regular archive inside the job directory; registry failure surfaces; never access BUILD_DIR; after direct-child exit 0 the orchestrator clones the archive out of the job directory into the operation directory and records its digest from that clone; later jobs read only that clone |
| resolve local | absolute regular `.tgz` from `--artifact`; mutually exclusive with tag | canonical path and archive SHA; clone into the operation directory and record the digest from the clone; no reinterpretation as directory/source checkout |
| extract | confined job: archive list first, then `tar -xzf <archive-clone> --strip-components=1 -C <extract-job-dir>/package` | all members rooted `package/`; reject absolute/.. names, hardlinks/symlinks escaping root and unexpected archived operator-state paths; validate canonical extracted files; no shell command construction; never writes `.hive.next` |
| validate stage | `readRelease` of the extraction output; manifest/root dependency maps/lock hash; expected service entries | reject unknown contract and missing worker before touching services |
| install | clone the extraction output into a fresh install job directory; confined `npm ci --omit=dev --no-audit --no-fund --no-progress` there, same recorded npm as validation | do not ignore scripts; bounded process with explicit failure classification; never borrow dev/global node_modules; non-zero direct-child exit disposes the job directory and fails before any signal |
| promote | after install exit 0, clone the install job output into absent `.hive.next` with the preflight-fixed method | any non-zero promotion exit discards the partial `.hive.next` and fails before any signal; method never switched; `.hive.next` is written only here |
| clone verification | reads the clone only: required entries, manifest/lock versus the marker's archive digest and the clone's shrinkwrap, confined dependency-tree check, Hive-owned `pkg/` entries hashed against archive members (tarball path only — update/first adoption; init/resume and tooling verification keep their own lists), confined runtime-loading check (offline diagnostic plus engine validate-only with dummy `HIVE_HOME`/`HIVE_CONFIG`) | any failure discards the clone and fails before any signal; success recorded in the marker as clone-verified |
| config preflight | staged `runtime-probe config` from the verified clone as a confined job under the intended service environment | configured instance/database identity, selectors, voice flags/ports compatible; required secret names resolvable; no call/vendor mutation |
| prior validation | captured effective running profile, compatible prior artifact/worker/deps (in-process `readRelease`, worker capability and containment of the promoted release; no packaged diagnostic subprocess) OR registered pilot snapshot | fail before stopping if recovery unavailable; mere `.hive.prev` existence is insufficient |

Chunk 5 Task 8 Step 1a.3 is the authoritative artifact-job contract for these operations and for bootstrap tooling: the launcher/profile/self-test, single-use job directories, clone promotion and verification, settlement by direct-child exit 0 plus verified clone, and next-invocation reconciliation. There is no unconfined `execFile` fallback for an artifact job, and no descendant accounting, guardian or settlement receipt. Service probes against promoted or running releases (health, worker maintenance, pilot evidence) are not artifact jobs and stay unconfined (runtime chunk Task 6 Step 2).

Stage while existing pair remains up. If `.hive.next` exists without this operation's ownership record, fail/reconcile rather than delete it; an unverified `.hive.next` recorded by an interrupted operation is discarded by reconciliation. Do not clone over a symlink or an existing destination. Native install has its own bounded staging budget (10 minutes); no service signal occurs during it. This budget is distinct from maintenance/health budgets.
...
The installed-worker `inspectIdle` subprocess is explicitly the private `worker-maintenance` interface in [chunk 5 Task 9 Step 4b.2a, Step 2b](./kpr-463-plan-pilot-boundaries.md). Bind its selected runtime, acquired owner, baseline/post-close/final phase, independent process generation and original deadline through `maintenanceQuiescenceIO`; final SDK/status/own-loader Mongo evidence reaches the actual `beforeExec` fence. Public diagnostic/activation `worker` remains open/null-owner health. This applies to normal packaged restart/update/rollback and C quiescence during pilot recovery, without a pilot registration. It is an unconfined service probe, not an artifact job.
...
`recoverPriorPair` first stops only positively identified candidate services (worker before engine) and verifies supervisor exit and, for the worker, listener release. It then restores original artifact positions and exact definitions/link state, starts engine, requires fresh boot, starts the prior worker, then checks the captured packaged or pilot profile. A bootstrap/plist/link/rename/health exception all enter this path; shell `set -e` cannot bypass it because the supported transaction runs inside this bundled helper. If a process cannot be stopped or a file move cannot be reconciled, do not start a conflicting prior pair on occupied ports; retain and report both failures.
...
| missing `/usr/bin/sandbox-exec`; self-test that does not deny; job direct child exits non-zero; clone verification failure (missing entry, manifest/lock digest mismatch, inconsistent dependency tree, runtime-loading failure); promotion exits non-zero mid-copy under either method; `cp -c` unsupported at preflight | fail closed before any signal; job directory disposed (deferred) and never promoted; partial or unverified `.hive.next` discarded; preflight-selected full copy used for the whole operation and never switched after a mid-copy failure; nonzero result |
| frozen helper killed during a confined job, during promotion, or after promotion before verification is recorded | next invocation (after lock-owner reconciliation) discards the unverified `.hive.next`, disposes leftover job directories (a sweep failure is reported, not fatal), never adopts a job directory, leaves the prior pair untouched and does not start the newly requested operation |
| ordinary rollback with `sandbox-exec` absent or non-cloning volume | rollback preflight, stop/start order and health unaffected; no confinement prerequisite consulted |
...
| worker supervisor stop timeout, health listener not released or held by a foreign owner, engine stop timeout | checked recovery or unresolved; never kill the listener owner; never claim reversible cancellation after signal; a surviving non-listener SDK helper alone does not fail the stop |
...
Keep existing developer helpers/tests scoped to developer mode, including any historical build fallback, with one removal: delete `kill_ports` (which pipes `lsof -i :<port> -t` into `xargs kill -9`) and its three developer-mode call sites, because it contradicts spec §5.1's "never kill arbitrary port owners". Where those call sites wait for a port, a port still owned after the existing wait reports the owner PID/command and fails that developer deploy instead of killing it; extend `service/deploy.test.sh` with a fixture owner that must survive. Extend `service/deploy.test.sh` with full supported-branch invocations using a helper shim plus transaction fixtures: no BUILD_DIR/instances.conf reads, no old installed updater calls, accurate nonzero propagation, dry-run zero writes, worker lifecycle covered by real helper fixture tests. Do not mistake old shell helper smoke success for pair-lifecycle success.
...
- [ ] **Step 4:** Implement/document first-adoption bootstrap into durable tooling at `<instance>/.hive-state/tooling/<archive-sha>/` (verified in a `.hive-state/tooling/.staging/<archive-sha>.<operation-id>/` sibling and atomically renamed; the final name is the only verification marker) using [chunk 4 Task 9 Steps 4a–4d](./kpr-463-plan-pilot.md) and chunk 5 Task 8 Step 1a.3. These substeps replace the earlier draft-only bootstrap heredoc and unspecified inventory-file contracts. They define strict serialized pilot/hold/bootstrap records and registration, allowed read-only check representations, ownership/hash/same-instance/path/process checks, complete dispatch-source and outstanding-work accounting, current verification/freshness/release procedures, stable bootstrap flags and pilot reconstruction/recovery/reapply. Preserve independently reviewed candidate SHA/revision and the complete validated tooling/dependency tree/archive through rollback/reapply; live service definitions point only at `.hive/pkg`.
...
- [ ] **Step 5:** In S9, add the adoption test with a deliberate old-updater sentinel script in target `.hive/service/deploy.sh` that writes `OLD_UPDATER_CALLED` and exits 97. Run the actual S8 candidate CLI from the bootstrap fixture. Require the sentinel never runs; candidate helper hash equals bootstrap package helper, both new plists point to target `.hive/pkg`, pilot snapshot survives, failure restores the pilot profile, and reapply needs a freshly verified legacy hold. Apply chunk 4 Task 9 Step 5a's explicit capable versus uninstrumented fixture distinction; the latter must defer without a fabricated capability. Also run the spec T6 tooling cases through the actual helper: a first adoption killed after the tooling clone but before the atomic rename, whose re-run discards the staging sibling and reuses nothing partial; and a second adoption of the same candidate digest, which reuses the final-name entry after re-checking its required entries and `pkg/release.json`, without reinstalling. After successful migration, ordinary future updates/rollbacks must work with independent packaged prior releases without reading pilot paths.
...
- [ ] **Step 2:** Document Node installed host path, supported engine range, operational Node24/macOS ARM64 proof, recorded npm version, command-line/build/native install prerequisites determined by the real T2 install, network registry requirement, no ignored native scripts, larger voice-disabled dependency footprint, instance health-port override and collision behavior. Document the staging host prerequisites for init/resume/update/first adoption: `/usr/bin/sandbox-exec` passing the fail-closed self-test (deprecated by Apple but present on macOS 26; the self-test, not presence, is the guard), and an instance volume supporting `cp -c` clone or the full-copy fallback with sufficient free space; state that ordinary rollback and running services need neither.
- [ ] **Step 3:** Document exact update/artifact/dry-run/start/stop/rollback commands, admission deferral/release-unresolved messages, retained ledger diagnostics, staging/recovery paths (`.hive-state/jobs/` single-use job directories with deferred disposal and non-fatal reported sweep failures, `.hive.next` clone verification, `.hive-state/tooling/` final-name entries and `.staging/` siblings with their retention and explicit cleanup step), first-adoption bootstrap and separate pilot recovery profile. Explain no automatic clearing of unknown accepted work and no force-call-termination option.
...
- [ ] **Step 1:** Retain existing Node22 checks and declared engine support. Add a macOS ARM64 Node24 packed-artifact check using the same check:artifact script; make the existing release validation path run it rather than relying on symlink runtime smoke. Existing CI host is self-hosted macOS ARM64; explicitly select Node24 for this job and retain a Node22 build/test/bundle job. Each fresh job builds before SDK-child tests and builds the complete bundle before artifact checks. The macOS ARM64 jobs run T2 and T7's detached-writer case unskipped under the real `sandbox-exec`, and the Node24 job also runs `node scripts/check-confinement-closure.mjs` as a T10 engineering check (the Node22 job reports T10 as a named skip). Increase job timeout only enough for fresh production native installs (30 minutes); do not skip the guard because it is slower. Publishing must depend on successful check:bundle/artifact validation and a clean manifest, while its existing publication authorization remains unchanged.
...
Expected: all exit 0; no skipped T1–T8 cases on the macOS ARM64 jobs; actual confined fresh install (zero denials), native, SDK-import and engine validate-only markers; T10 runs unskipped and passes on the Node24 macOS ARM64 job; the named T10 skip applies only to the Node22 job (and any non-24 or non-macOS host) and never counts toward closure. Inspect the tarball listing and release/lock identity from the built artifact. Confirm no source/dev/global dependency links in reported runtime paths. Run Node22 and Node24 matrices as specified; an unavailable required Node24 ARM64 runner is a concrete T2 blocker.
...
`KPR463_BOOTSTRAP` is the verified final-name tooling entry `<instance>/.hive-state/tooling/<archive-sha>/` printed by the bootstrap; never a `.staging/` sibling or a job directory. The former manual unconfined `voice-worker-diagnostic offline` invocation from tooling is removed: the bootstrap's tooling verification subset already validated this entry, and the update's own confined clone verification runs the worker diagnostic and engine validate-only check against the candidate clone.
...
The helper's first migration path reads the registered pilot snapshot and current verified hold record under this instance (an explicit `--legacy-hold` selects a registered record, never an arbitrary bypass); require explicit selected instance agreement and clean archive identity. No implicit old updater or BUILD_DIR fallback. Expected: confinement self-test passes and its outcome, macOS version, promotion method, per-job exit statuses, job directories and clone-verification result are recorded; candidate `.hive`, engine fresh boot then worker registration, all packaged checks pass, no message/call emitted. If any operation fails, retain primary/recovery evidence and leave acceptance pending.

- [ ] **Step 3a:** On this deployment-proof host, run the T10 closure harness from the reviewed repository revision against the exact selected archive and copy its record into the evidence file. It touches no instance service, operator configuration or Keychain.

```bash
node scripts/check-confinement-closure.mjs --artifact="$KPR463_ARCHIVE"
```

Expected: a closure record for T10(a)–(e) with the host's macOS version and exact Node/npm versions written into the evidence host/runtime field. If the invoking Node's major version is not 24, the harness reports a named skip: record it, leave T10 pending with that reason, and do not treat any other run (including CI) as closing T10.
...
| --- | --- |
| execution | date/operator, real host OS/arch, macOS version and exact Node/npm versions on the deployment-proof host (the authoritative value T10's Node 24 major-version gate is diffed against) |
| artifact | source revision, package version, dirty=false, archive/lock digests, retained archive/tooling path |
| staging integrity | confinement self-test outcome with macOS version; per artifact job the profile applied, job directory and direct-child exit status; promotion method and clone-verification result for the activated `.hive.next`; confirmation that no job directory was promoted; any reported sweep failure |
| T10 closure | Step 3a closure record for (a)–(e), or the named skip/pending reason |
...
- KPR-462 now carries canon R1–R12; none binds KPR-463's deployment content directly. Shared main/config/telemetry changes must be integrated serially with later KPR-464/KPR-465 work.
- The staged candidate `runtime-probe config` resolves real config and Keychain names as a confined job; Seatbelt denies only file writes, so Keychain reads through the system security service are expected to work. S9 exercises the confined probe with the fake `security` shim; the real Keychain read under confinement is first observed in S12, and a failure there is a concrete blocker to report rather than a reason to run that probe unconfined.
- Artifact jobs are the staging-time subprocesses that produce, install or validate an unpromoted tree (candidate clone or tooling staging sibling); service probes against promoted or running releases are not artifact jobs. This reading is what makes spec §4/§5.2's rollback independence from `sandbox-exec` consistent with §5.1's "any other packaged probe".
