...
| clone verification | reads the clone only: required entries, manifest/lock versus the marker's archive digest and the clone's shrinkwrap, confined dependency-tree check, Hive-owned `pkg/` entries hashed against archive members, confined runtime-loading check (offline diagnostic plus engine validate-only with dummy `HIVE_HOME`/`HIVE_CONFIG`) | any failure discards the clone and fails before any signal; success recorded in the marker as clone-verified |
