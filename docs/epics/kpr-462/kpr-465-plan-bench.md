...
- [ ] **Step 5: Bench invocation (from the instance dir so `config.ts` finds `hive.yaml`/`.env`/Honeypot).** **Warm-arm spacing (pre-PR review round 2 finding, corrected at round 3):** each warm bench call holds one of the clone's `spawnBudget` (5) slots for the call plus the engine's `WARM_IDLE_TIMEOUT_MS` (120 s) after its last turn; running `--calls` sequential warm calls with no gap can exhaust the budget before earlier leases idle out (503s misread as a real finding, and a live kill drill's `pgrep` picking up a still-open prior call's process instead of the drilled one). **`--drill double-request` is NOT exempt from this** — like the plain multi-call invocation, it defaults `--calls` to 8 and runs 8 full sequential bench calls (each one doing its double-request pair on turns 1–2, per `resolveCallsCount`/`runBenchCall`); only `concurrent-3` (a fixed 3 parallel calls, ignores `--call-gap-ms`) and a kill-* drill (defaults `--calls` to 1) are naturally under the budget. So: use `--call-gap-ms` on the plain warm block AND on `--drill double-request` — at least `130000` (120 s + margin) is the safe default absent a faster empirical read from the session's own block timing; **not needed on the cold arm** (no warm lease, no budget held between calls). Between separate warm-arm invocations (including before `concurrent-3` and before each kill-* drill), wait for a fresh quiescence check (`warm-voice=0`, no running claims — the same check as Task E3 Step 2) rather than assuming the prior command's own internal spacing was enough. **`--call-gap-ms` does NOT cover the `--warmup` boundary** (pre-PR review round 4 finding — the code only sleeps between sequential scripted calls, never before the first one, so the warm-up call runs immediately with no gap regardless of the flag): the warm-up's own lease still occupies a slot for up to 120 s into the block, meaning V1's "`warm-voice=1` during" check will legitimately read `2` on the block's first scripted call after a `--warmup` — this is expected, not a bug, and V1 should be scored on a LATER call in the block instead, once the warm-up lease (a distinct lease under its own call id, never sharing the scripted calls' lease) has had time to idle out. **Note (pre-PR review round 5):** at a `--call-gap-ms` of ~130s against the 30s spawn-coordinator heartbeat window, the `warm-voice=0` half of V1 ("within 120s of the last result") will typically be visible for only ~10s between calls and the heartbeat may miss it on most calls — rely on the "Warm voice lease closed … idle-timeout" log line (V5) as the per-call reclaim signal, and don't treat a heartbeat snapshot that landed outside that window as a V1 failure:
...
# warm block: --call-gap-ms spaces every SEQUENTIAL scripted call past the 120s warm-lease idle timeout (not the --warmup call itself, which runs before the loop with no gap)
npx tsx ~/github/hive/scripts/voice-engine-bench.ts --arm A1-warm --out /tmp/kpr465/A1-warm.jsonl --calls 8 --call-gap-ms 130000 --warmup
# re-check warm-voice=0 (Task E3 Step 2) before each of the following — double-request is 8 full sequential calls too, not one:
npx tsx ~/github/hive/scripts/voice-engine-bench.ts --arm A1-warm --out /tmp/kpr465/A1-warm-dbl.jsonl --drill double-request --call-gap-ms 130000
