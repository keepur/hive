...
- [ ] **Step 4: Per block (A0 cold, A1 warm, A2 warm+effort):**
  ...
  2. `--warmup` discarded turn, then ≥ 8 scripted calls (`--calls 8`), then the drills on the warm block: `double-request` (V9), `concurrent-3` (V10), `kill-a`, `kill-b`, `kill-c` (V6 a/b/c — the operator supplies the pid at the prompt; V6(c) is recorded as the *no-retry* outcome, not a failure).
...
| V3 tools | lookup turns' `toolCount ≥ 1` (`benchAssertions.tool.pass === 2 × calls`), `toolAckInjected: true` where the model was silent |
