...
- Repeat KPR-464 L1–L4 on this deployment (canon R5). Frozen 464 live rows are `pending` and are not a 466 pass. KPR-465's offline snapshot is not L1–L4 (R12). L2 requires opening suppression only when nonempty final or accepted-turn is observed before the opening request; a late-final greeting that yields one opening then replacement is L2 `pass` (R6; 464 §3), whether the accepted-turn hook cancelled that opening (`startup_superseded`) or the SDK had already interrupted or settled it (no cancel marker). Copying the frozen 464 “no optional opening” cell does not override that.
...
**Capture (not experiment):** on 466's calls, record `voice.warmPath.enabled`, Mokie's `effort`, endpointing presence, and schema-v2 JSONL `warm` / `selectedContinuity` only (`EnginePayload` in `src/voice/voice-trace.ts`; the diagnostic reader has no `warmTurnSeq` allowlist). `warmTurnSeq` is on the engine `Voice turn complete` line (`src/channels/voice/voice-adapter.ts`) and the compare `--engine-log` path (`parseEngineLog` in `src/voice/voice-latency-compare.ts`), not in JSONL. A warm-labelled run whose steady turns are not `warm: true` fails capture. A keep-cold run containing any `warm: true` steady turn fails capture. Capture fail remains that `warm: true` consistency check, not the `warmTurnSeq` key. Confirmatory stage tables for I1 use `scripts/voice-latency-compare.ts` on those call ids only. No A0/A1 pairs, no 36-turn sample plan, no bootstrap claim.
...
| H | Hangup cleanup (mid-call) | Normal goodbye then hangup on I1 | `call_closed`; every started attempt terminal or explicit incomplete; **no** `speech_started` / engine write / generated frame after close; worker `activeCalls` on `voice_worker_stats` returns to the pre-call value on a document with `updatedAt` after hangup — `activeCalls` is decremented by `noteCallEnded` at job teardown (`src/voice-worker/session.ts` `releaseCall` / `runJobShutdown`; `src/voice-worker/telemetry.ts` `noteCallEnded`), not by the 30 s supervisor `writeOnce` (that tick `$set`s `cellDefaults`/`updatedAt` only and never the call counters; `VoiceWorkerHeartbeat.INTERVAL_MS`); wait for that post-hangup `updatedAt` rather than sampling immediately; if warm, observe until `warmVoiceSessions == 0` or `WARM_IDLE_TIMEOUT_MS` (120 s from last turn result, `src/agents/warm-voice-session.ts`; V5: idle timer arms at last result, not at hangup) | No late speech after hangup | Machine-only rows may `pass` without audio; “no late speech” without a caller is `unknown` |
...
**L2 observed ordering (R6; 464 §3; `startup-arbiter.ts`).** Classify Call 2 from content-free JSONL only: `caller_final_input.hasFinalInput`, `opening_decision` `decision`/`reason`, `caller_turn_accepted`, speech `origin: "opening"` / `speechId`, `speech_terminal` `outcome`/`cause`, and cancel cause `startup_superseded` only when application cancellation occurred. Do not read transcript or prompt text. `callerState("listening")` reevaluates startup without waiting for final; `finalInput` latches nonempty final only; `acceptedCallerTurn` consumes the opening decision and interrupts only the retained opening handle if it is still outstanding; the SDK may already have interrupted it (464 §3). Late-final (`quiet_answer` request, then nonempty final, then accept) is `l2_late_final_replaced` when application cancellation wrote `startup_superseded`, and `l2_late_final_sdk_interrupted` when the SDK had already interrupted or settled the opening so that marker is absent.
...
| `l2_late_final_replaced` | Listening reevaluated with no final pending, so one `quiet_answer` opening was requested; nonempty final then arrived; application cancellation of that still-outstanding opening (`startup_superseded` on its `speechId`). Includes reentrant accept during `requestOpening`, and an SDK-interrupted handle that had not yet settled so application cancellation still wrote the marker | `pass` when exactly one opening is requested then superseded, exactly one caller-owned replacement proceeds, that opening is not queued behind the replacement, and no extra utterance is required to progress. Caller may hear a brief opening then the hello-response (`heard_both`); that is not L2 `fail` and is not a §3.1 duplicate/stale opening |
| `l2_late_final_sdk_interrupted` | Same request-then-late-final-then-accept ordering, but the opening was already interrupted or settled by the SDK before acceptance, so no cancel marker is written (arbiter skips `done()`/`interrupted` handles; a settled handle is already gone from application ownership). Identify that opening by its `origin: "opening"` `speechId` plus its `speech_terminal` ordered before `caller_turn_accepted` / `opening_decision` `consumed`+`accepted_caller_turn` — `outcome: "interrupted"` (cause may be `unknown`) when the SDK interrupted it; `outcome: "completed"` when it settled without interrupt. Do not require `startup_superseded`; that path never writes it. Do not identify it by ordering against a speculative replacement `speech_started` (preemptive generation may create that handle first) | `pass` when exactly one such opening is requested, exactly one caller-owned replacement proceeds, that opening is not queued behind the replacement, and no extra utterance is required to progress. Caller may hear a brief opening then the hello-response (`heard_both`); that is not L2 `fail` and is not a §3.1 duplicate/stale opening |
...
Any of `l2_suppressed`, `l2_late_final_replaced`, or `l2_late_final_sdk_interrupted` completes required L2 coverage. Call 2 remains required; a late-final pass does not absorb L3.
