...
- Consume KPR-463's packaged-release evidence (`docs/epics/kpr-462/kpr-463-deployment-evidence.md`, spec §6/§7/T9) and **rerun** its no-call preflight on the identity that will actually place the calls. Do not re-execute restart/rollback as a 466 experiment.
...
- Smallest live set: one initiation call that also covers quiet opening, exchange, barge-in, tool-ack, and hangup (I1), plus L2, L3, L4. Voicemail is classified when observed, not manufactured. No AMD, no vendor API probes, no numeric SLO, no CNAM, no inbound.
...
- Blocking for execution only: May's explicit live-call go, a 463 T9-passed dodi identity, and the 465 configuration input above. Drafting authorizes none of those.
...
| `src/contacts/contacts-mcp-server.ts` | `contacts_search` / `contacts_get` return stored E.164 among other fields. | Resolution is an agent behavior, not a new API. The user request must not include digits. |
...
| `fail` | Caller reports a defect, or machine evidence shows a defect on a call the caller also experienced (wrong destination, late speech after hangup, duplicate/stale opening, tool never ran when claimed, leftover warm session). |
...
`callerConfirmation` in JSONL stays `"unknown"`. Human verdicts live only in the evidence record, keyed by call id and observation time, never back-filled from timestamps.
...
| `live end-to-end acceptance passed` | Preflight green, every required scenario `pass`, voicemail `pass` or `unobserved`, consumed restart/rollback `consumed` or `pass` | **Yes** |
...
2. Reruns the §6 packaged-release no-call profile on the **current** live PIDs/args/paths (an intervening update invalidates the earlier identity).
...
**Preferred sequence (waterfall):** 465 E6 exists. 466 live runs on that applied setting. Same-session batching under one May go is allowed: finish the 465 decision (or confirm keep-cold), then run 466 scenarios. 465's live calls are **not** 466 L1–L4 (465 turn 1 is a scripted greeting, not the SIP opening — 465 plan, R6).
...
| T | Harmless read-only tool + ack | One I1 turn that forces a read-only lookup Mokie actually carries (prefer `conversation_search` if present; `contacts_search` is acceptable). Not `voice_call`, not a write (`contacts_create`/`contacts_update`, browser, code-task, …) | `toolCount ≥ 1`; `toolAckInjected` true **or** caller heard an acknowledgement when the model was silent; conversation resumes (`maxInterChunkGapMs` recorded if present) | Heard an acknowledgement (or a brief spoken hold) and a spoken result, then normal talk | `unknown` |
| H | Hangup cleanup (mid-call) | Normal goodbye then hangup on I1 | `call_closed`; every started attempt terminal or explicit incomplete; **no** `speech_started` / engine write / generated frame after close; worker `activeCalls` returns to the pre-call heartbeat; if warm, `warmVoiceSessions == 0` within `WARM_IDLE_TIMEOUT_MS` (120 s) of the last turn result (V5 timing: idle timer arms at last result, not at hangup) | No late speech after hangup | Machine-only rows may `pass` without audio; “no late speech” without a caller is `unknown` |
...
| C | Warm/cold capture | None beyond running on the 465 setting | §4.3 consistency check + confirmatory compare CLI on I1 call ids, R8 split honored | Pause verdict on I1 (acceptable / borderline / annoying) | Pause row `unknown`; machine consistency may still `pass`/`fail` |
...
| Class | Machine | Caller | Scores L1/X/B/T/H? |
| --- | --- | --- | --- |
| `no_sip_answer` | No `sip_answered`; setup failed or createSipParticipant never completed | No ring, busy, or gave up | No — not a conversation call |
| `answered_voicemail` | `sip_answered` (SIP 200) | Heard voicemail / greeting-machine, no human | No — V `pass` if classified; do not score L1 as fail |
...
If I1 lands voicemail or no SIP answer, keep it in the denominator as V, do not score L1/X/B/T/H on it, and a retry of I1 under the **same** go is allowed (intended destination, classified outcome). An **unexpected** destination (wrong person) trips the stop rule and is not retried under that go.
...
Repeats and failed attempts stay in the denominator; they are not dropped. Do not import the KPR-465 10-turn bench script onto the handset — it is an experiment instrument (powered n, `mokie-bench` clone, “bench marker”). I1 is short on purpose.
...
2. KPR-463 T9 passed and P1–P6 green on the identity that will dial, recorded in the 466 evidence file.
...
466 is not an A/B. After each human-answered call, May records: first audible words; pauses (acceptable / borderline / annoying) on I1; cut-off or talked-over; barge-in quality; duplicate/stale speech; tool acknowledgement heard; late speech after hangup; per-turn correctness for X and T; free text (verbatim only with her ok). Observation time is stored with the call id. Disagreement between machine rows and the verdict is reported, not reconciled away.
...
2. Build/pins and running identities (engine, worker, package version, archive/lock digests from 463, `@livekit/agents` 1.6.4, Node, UTC clock).
3. 465 configuration input and readback (warm / effort / endpointing / keep-cold-by-absence).
4. P1–P6 preflight table.
...
- **Non-blocking, delegated:** keep-cold by absence when 465 has no §6.4 entry; 466 does not itself choose warm or effort.
- **Non-blocking, delegated:** voicemail `unobserved` is allowed; distinguishing when it occurs is required.
- **Non-blocking, delegated:** Slack is the user-request channel; contacts or memory may resolve the number; the request must not contain digits.
...
- **Blocking for execution only:** May's live-call go; 463 T9-passed identity; P1–P6 green. Without those, stop at `no-call preflight verified; live end-to-end acceptance pending` (or earlier, if 463 T9 itself is pending).
