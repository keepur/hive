...
These modes observe the configuration and health of an already promoted or running release; they are service probes, not artifact jobs, so they run unconfined and need neither `sandbox-exec` nor clone capability, which keeps ordinary rollback's health checks independent of those host prerequisites (spec §4, §5.2). The one exception is the staged candidate's `config` compatibility probe during init/resume/update/first adoption, which runs from the verified clone as a confined job under chunk 5 Task 8 Step 1a.3 (reads and network allowed, writes confined to its job directory).
