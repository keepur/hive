...
**Retain an early failure until both exact binding and action admission resolve.** Keep `pendingErrors: Map<turnId, BridgeError>` capped at 256 plus a weak identity deduplicator for repeated delivery of the same Error. A callback records each local failure immediately. For an exact BridgeError, insert it once then call `recheckPending(turnId)`. This function reads `trace.bridgeBinding(turnId)`; `unbound` stays pending, `unavailable` is recorded unproved and removed, and `bound` reads `admissions.get(binding.speechId)`. No admission yet means retain it even when its immutable creation epoch is older than the accepted hook. A present but revoked/older admission is stale and removes the error without consuming retry budget. A current owner removes/latches the error **before** registering its single recovery chain. Store `origin: Object.freeze({ turnId, speechId: binding.speechId })` on that chain before any foreign callback or await; this is the failure's original bridge binding, independent of the chain's transferable action owner. Queue the chain body with `Promise.resolve().then(...)`, rechecking `chainOwns` in that continuation before running recovery, so no fallback is emitted reentrantly inside the SDK metrics callback. The error's failure classification/terminal evidence is never removed from the recorder by action decisions.

Subscribe once before start with the `recheckRecoveryForTurn`/`recheckPending` listener below; also recheck pending errors on initial capture and after exact EOU/application admission. These cover error -> binding -> EOU, EOU -> error -> binding, and binding -> EOU -> delayed session Error, without an event-order race. Keep a bounded 256-entry processed-turn decision cache plus weak Error identity deduplication; conflicting errors for the same turn cannot spend twice. When a creation/admission becomes known stale, drop its pending errors with unproved/stale action evidence. Before evicting the oldest pending error on insertion 257, read its own binding: `trace.actionGap("action_overflow", binding.state === "bound" ? binding.speechId : null, evictedTurnId)`. Keep the original turn ID when unbound; do not invent a speech. Remove/mark that pending decision so later notifications cannot reschedule it. No missing binding, creation, EOU or cache entry ever authorizes fallback/shutdown. There is no timer waiting for optional metrics and no wait on this registry in the media path.
...
interface RecoveryChain {
  ...
  bindingInvalidationHandled: boolean; // Initialize false; independent of signal.aborted.
  owner: ActionOwner;
}
...
const originStillBound = (chain: RecoveryChain) => {
  const binding = trace.bridgeBinding(chain.origin.turnId);
  return binding.state === "bound" && binding.speechId === chain.origin.speechId;
};
const chainOwns = (chain: RecoveryChain) =>
  activeRecovery.get(chain.id) === chain && !chain.abort.signal.aborted &&
  owns(chain.owner) && originStillBound(chain);
...
Keep the originating binding for the chain's full lifetime, including after `pendingErrors.delete`, handle settlement and intentional owner transfer. Revalidate it in `chainOwns` before **each** budget consumption, wait construction, delay, `say`, `generateReply`, flush/release/shutdown, and after every await. Binding conflict, last-lookup eviction or recorder close conservatively revokes the chain even if its current action owner is still admitted. An atomic active-to-recent move preserves authority; actual eviction does not. Do not retain a private cached `bound` result to bypass eviction. Cancellation is sticky: late rebinding cannot revive a canceled/evicted chain or spend its processed failure again.

Install this one bounded subscription before session start and dispose it at call cleanup. `recheckRecoveryForTurn` is also callable directly in unit tests. It scans the deduplicated union of active routines and chains retained by unfinished application handles. Initialize `bindingInvalidationHandled` to false before registering each chain; generic continuation abort must neither set it nor stand in for it. A speculative creation may abort the routine while leaving its queued fallback retained, and routine `finally` may then remove the active entry before binding loss arrives. Binding invalidation must still interrupt that handle. Latch binding invalidation before any abort/trace/interrupt callback, so repeated or reentrant notifications handle it once. Never reset the latch on owner transfer or late rebinding. The listener cancels the controller immediately during a retry delay or after the routine's `finally`, and interrupts only application handles belonging to that exact chain, so already-queued stale fallback/retry output is suppressed:

```typescript
function recheckRecoveryForTurn(turnId: string): void {
  const retainedChains = new Set(activeRecovery.values());
  for (const entry of applicationHandles.values()) {
    if (entry.chain) retainedChains.add(entry.chain);
  }
  for (const chain of retainedChains) {
    if (chain.origin.turnId !== turnId || chain.bindingInvalidationHandled || originStillBound(chain)) continue;
    chain.bindingInvalidationHandled = true;
    cancelChain(chain);
    trace.actionGap("action_ownership_unproved", chain.origin.speechId, turnId);
    for (const [speechId, entry] of applicationHandles) {
      if (entry.chain !== chain) continue;
      trace.markCancellation(speechId, "unknown"); // Binding loss proves no barge-in cause.
      if (!entry.handle.done() && !entry.handle.interrupted) {
        try { entry.handle.interrupt(); }
        catch { trace.actionGap("cancel_failed", speechId, turnId); }
      }
    }
  }
}
const detachBinding = trace.onBridgeBinding((turnId) => {
  recheckRecoveryForTurn(turnId); // Includes chains whose pending error was removed.
  recheckPending(turnId);
});
```

The maps are bounded at 256 each, so this per-notification set contains at most 512 distinct chains and is discarded when the callback returns; no persistent index extends their lifetime. A retained chain becomes unreachable after its active routine and last owned handle entry are removed. Deduplication plus the separate sticky `bindingInvalidationHandled` latch gives one binding-loss gap per chain across both maps and repeated notifications, including chains whose controller was already aborted for another reason; mark cancellation before interrupting, and leave terminal outcomes/counts to the existing one-shot handle/attempt finalizers. An already-aborted controller emits no second abort event, but its unfinished retained handles still require binding-loss cancellation. Recorder mutation installs unavailable/conflict state **before** this callback. A registration-to-microtask invalidation aborts the just-registered controller; the microtask must then emit no action and spend no retry budget. A delay-time invalidation aborts the timer without needing a retained handle; chain finally removes the entry. A queued retry remains subject to binding invalidation after that finally removes its routine: interrupt the retained handle synchronously before releasing the fake-output gate. Even if a read detects invalidity before its notification, `chainOwns` fails closed. Preserve original terminal evidence and explicit gap identity; canceling authority never rewrites the historical failure.
...
Add binding invalidation cases with the real `SpeechTrace` and production recovery controller: register a valid bound/admitted failure, assert it has left `pendingErrors` and entered `activeRecovery`, then inject conflicting binding synchronously before the queued chain microtask; zero `say`/retry/budget/shutdown. Repeat during the post-fallback delay after its handle is done/removed: controller aborts, timer settles immediately and no later action occurs. Repeat true recent-cache eviction (257 distinct entries), recorder close, and invalidation after an intentional fallback transfer: check the original bridge identity still governs, queued chain-owned handles interrupt, and an unrelated healthy successor remains untouched. An active-to-recent move and duplicate unchanged binding preserve the live chain; a late original-ID metric after conflict/eviction cannot revive it. Add 257 unbound pending BridgeErrors: exactly one oldest-turn overflow gap has `speechId: null` plus its real turn ID, 256 pending entries remain, no invented speech/action and no throw. These focused cases use named gates/fake timers; no live SDK/provider is required to validate registry authority.

Add the explicit post-routine schedule: valid admitted failure → allowed retry `generateReply()` returns an unfinished application handle behind a first-frame gate → drain the recovery promise and its observed catch/finally → assert the chain is absent from `activeRecovery` while that exact handle/chain remains in `applicationHandles` → inject binding conflict, actual 257-entry recent eviction, or recorder close → assert synchronous one-time chain abort, one `action_ownership_unproved` gap with the original speech/turn IDs, and cancellation before one handle interrupt → release the frame gate and observe zero stale frames. Repeat notifications and late original-ID metrics; neither revives authority nor duplicates the gap, interrupt or terminal counts. Assert the existing failure outcome survives and an unrelated healthy successor progresses untouched. On public done, remove the exact retained handle entry; after settlement/eviction/cleanup neither map retains that completed chain. Also run a valid binding move/duplicate control: the queued retry remains uninterrupted and reaches its known frame even after routine removal. Test a chain present in both maps (and multiple retained entries for that chain) so the union cannot duplicate cancellation/gaps; test handle-map overflow and call cleanup while only the retained-handle reference remains. Task 7 repeats the post-routine conflict/eviction ordering with the real SDK and fake output, not only scheduling spies.

Add the prior-abort permutation: valid admitted failure → owned fallback `say()` returns an unfinished handle behind a first-frame gate and `waitOwned` awaits playout → speculative SDK creation revokes the current owner and aborts the continuation → drain the abort-winning wait and routine catch/finally → assert `abort.signal.aborted === true`, `bindingInvalidationHandled === false`, no active entry, and one retained uninterrupted fallback → inject binding conflict, actual 257-entry recent eviction, or recorder close → assert the latch becomes true, one original-ID ownership gap and cancellation precede one handle interrupt, with no second abort event → release the frame gate and observe zero stale frames. Duplicate/reentrant notifications and late original-ID metrics do not duplicate gaps, interruption or terminal counts; no retry/shutdown follows and an unrelated successor progresses. Repeat with the same chain present in both maps and multiple retained handles. The valid-binding move/duplicate control after the earlier speculative abort must leave the retained older fallback uninterrupted and able to reach its known frame, while `chainOwns` stays false and no new recovery action is authorized. Intentional transfer with a still-valid origin keeps the chain's controller/latch and normal recovery progression. Task 7 repeats this prior-abort → finally → conflict/eviction ordering through production recovery, public SDK handles and fake output. These cases supplement the post-routine tests above and retain the same bounded-registry/cleanup assertions.
