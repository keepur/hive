...
function recheckRecoveryForTurn(turnId: string): void {
  ...
  for (const chain of retainedChains) {
    if (chain.origin.turnId !== turnId || chain.abort.signal.aborted || originStillBound(chain)) continue;
    cancelChain(chain);
