...
**Status:** spec-ready (spec review round 1 — approved, caught-by: spec-review/1/opus)
...
4. **Blast radius is exactly two consumers, both confirmed by grep.** (a) `dispatcher.ts:1421` — the KPR-388 delta-into-fresh mark heal, reached via `convertTurnResult`'s propagation at `dispatcher.ts:503` (`resumedSession: turn.resumedSession`); (b) `recordSpawnObservability` → `agent_turn_telemetry.resumedSession` (`agent-manager.ts:2217`). It is **not** read by `finalizeSpawnResult`'s persistence gates (`:2315`/`:2327` key on `result.aborted`/`result.sessionId`, never on this flag), by the outage queue, by deadline continuation, or by any adapter — `agent-runner.ts:161` documents the field as dispatcher-populated passthrough only.
...
- If the retry *fails with an error*, the tracker is still correct: the finalized (failed) attempt genuinely ran fresh, and the dispatcher's mark bookkeeping is gated on `!runResult.error` anyway (`dispatcher.ts:1419`); the telemetry record gate (`:2198`) additionally requires a truthy `result.sessionId`, so on a failed retry the value may be consumed by nothing at all.
...
All new rows go in the `TurnResult.resumedSession (KPR-388)` describe (`agent-manager.test.ts:3257`), whose existing rows already pin the other three arms. Fixtures come from the `resume-rejection self-heal (KPR-399 §D3)` describe (`:3586`) — `UNKNOWN_SESSION` and `DANGLING_TOOL_USE` — so both matcher surfaces are covered. **Hermetic queue reset:** unlike the KPR-399 describe (`:3591-3600`), the `resumedSession` describe has no `mockRunnerSend.mockReset()` `beforeEach` — the double-`mockResolvedValueOnce` pattern in T1 relies on the existing per-test queueing already established at `:3281` draining cleanly (pre-fix, both queued values still drain, so there is no bleed hazard either way), but the plan should carry this as an explicit invariant rather than something the implementer has to re-derive.
...
- **Interaction with the KPR-350 arm's contender adoption** — mutually exclusive by `else if`; the two matchers are pinned non-overlapping in both directions — `error-classification.test.ts:331-350` (a stale-server string does not match `isClaudeResumeLoadError`) and `agent-manager.test.ts:6003` (a claude-resume string, e.g. "conversation not found", is in `isStaleServerHandleError`'s `MUST_NOT_MATCH` set).
- **Full/summary-mode conference turn on this arm** — the fix flips `resumedSession` to `false` for these too, not only delta turns, but the dispatcher's clear branch is delta-exclusive (pinned at `dispatcher-conference.test.ts:1159` and `:1435`), so full/summary-mode turns are unaffected by the fix — full mode already re-injects everything, so a mark advance there is earned regardless of `resumedSession`.
