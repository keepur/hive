...
**Status:** spec-ready (spec review clean r2, fable; r1 blocking + 7 advisories resolved, r2 advisories folded).
...
- **⚠ Through-the-boss is enforced by tool gating, not prose — and the gating needs TWO layers because the runner auto-injects.** Stripping the cloned config's `coreServers` is a **no-op** for `schedule`/`team`/`team-roster` (and `workflow`/`skill-author`): `effectiveCoreServerSet()`/`filterCoreServers` (agent-runner.ts:410–432, :1068–1094) unconditionally `coreSet.add(...)` them regardless of the array. Layer 1 (required mechanism): an `AgentRunner` **worker-mode flag** set by the worker spawn path that gates those auto-injection lines, so a worker-flagged runner genuinely omits them. Layer 2: `WORKER_SERVER_DENYLIST` governs the non-auto-injected servers. Result: no outbound-message surface (direct or via `team`-relayed agents), no self-scheduling/re-entry minting, no recursive dispatch; the worker's only output channel is its return value, relayed by the boss (code-enforce-don't-prose-enforce). The containment pin must assert the runner's **built server set**, not the cloned config array.
...
- **Lane B visibility (r2 — explicit compensation REQUIRED, not free):** Lane B's bridged inventory comes from `buildToolTransportInventory` → `filterCoreServers(buildAllServerConfigs(...))`, and a brand-new in-process server appears there only via one of the two existing patterns — a vestigial stdio entry in `buildAllServerConfigs` (KPR-183 pattern, 9 of the 10 ported servers) or explicit compensation in the inventory builder (KPR-327's `memory` pattern: agent-runner.ts:245–250 comment + the special case near :1291). `worker-pool` MUST ship one of these (prefer the memory-pattern compensation — no fake stdio path to maintain) or Lane B bosses get no `worker_dispatch` at all. With that in place the KPR-348 `ToolBridge` carries the tools with no adapter changes; workers themselves are always claude-lane regardless of the boss's lane.
...
const runner = new AgentRunner(workerConfig, memoryManager, plugins, skillIndex,
  ...
  { workerMode: true });                                 // r2 REQUIRED — see "worker-mode flag" below
...
- **Worker-mode flag (r2 — REQUIRED mechanism; the denylist alone is not structural for auto-injected servers).** `effectiveCoreServerSet()` (agent-runner.ts:410–432) and `filterCoreServers` (:1068–1094) **unconditionally auto-inject** `schedule`, `team`, `team-roster` (plus `workflow` when enabled, and `skill-author` in `filterCoreServers`) regardless of the config's `coreServers` array — so the clone's filter is a no-op for exactly the servers whose absence the through-the-boss and no-self-scheduling claims depend on. Design requirement: an `AgentRunnerOptions` **`workerMode: true`** flag, set only by the worker spawn path, gates those `coreSet.add(...)` lines at **all** sync-mirrored sites (`effectiveCoreServerSet`, `filterCoreServers`, and `autoInjectedServerNames` for the toolkit section — the existing "keep both sites in sync" comment now covers three-way worker-mode agreement). A worker-flagged runner genuinely omits `team`/`schedule`/`team-roster`/`workflow`/`skill-author`; the denylist remains the second layer for the non-auto-injected servers. **Test consequence:** asserting the cloned config's `coreServers` array is insufficient — the pin must be on the runner's *built* server set (`filterCoreServers` output ∪ `buildInProcessServers` keys) containing none of the denied or auto-injected names (T3 updated).
- **Known limitation (recorded, follow-up ledger — not this ticket):** the fixed denylist does not cover vendor servers with outbound capability outside the named set — notably `google` (Gmail send via `gog`) survives the strip on a boss that carries it. The r1/r2 rulings chose denylist over allowlist deliberately (workers need the boss's read/search capabilities, which an allowlist would have to re-enumerate per instance). Trigger for the follow-up: any observed worker-authored outbound side effect, or the first plugin shipping an outbound-capable server — then revisit as an allowlist or a per-server outbound-capability trait.
...
- `src/agents/agent-runner.ts` — optional `workerPool` constructor dep; `buildInProcessServers` `worker-pool` block (context-ref refresh); **`workerMode` runner option gating the auto-injection lines in `effectiveCoreServerSet` + `filterCoreServers` + `autoInjectedServerNames` (r2)**; **Lane B inventory compensation for `worker-pool` (KPR-327 memory pattern, r2)**.
...
- **E5 Engine restart mid-worker:** restart sweep expires all `running` claims with notice; no worker survives by construction — the `background` denylist entry is load-bearing here (the denylist removes the *sanctioned* detached-execution surface `bg_execute`, so nothing a worker starts through its tool surface outlives the wall clock, `worker_cancel`, `stopAgent`, or the restart — this is tool-surface containment, not Bash-level containment: raw shell daemonization remains theoretically possible under `bypassPermissions`, as it is for every agent and delegate in hive); ledger (Mongo) survives, in-memory state rebuilt empty.
...
**Part A**
- **T1 (atomic claim):** two concurrent `worker_dispatch` handler invocations, identical task ⇒ exactly one claim doc; loser's response names the claimant. Table: distinct tasks ⇒ two claims; `perMeetingMax` reached ⇒ refusal; concurrent distinct dispatches straddling the cap ⇒ bounded overshoot tolerated (pin the check-then-act acceptance: the test asserts the overshoot is tolerated rather than a stricter bound; "no lock/re-count added" is a plan/review gate, not a test assertion — a unit test cannot assert code absence).
...
- **T3 (worker spawn shape + containment pin):** captured `AgentRunner` construction — model pinned from config, `workerMode: true` passed, `delegateServers: []`, `prefixCache` undefined; `runTurn` receives `sessionId: undefined`, worker limits, a `systemPromptOverride`. **The containment assertion targets the runner's BUILT server set** (`filterCoreServers` output ∪ `buildInProcessServers` keys on a worker-flagged runner): contains no denylist name AND none of `team`/`schedule`/`team-roster`/`workflow`/`skill-author` — a config-array-only assertion is explicitly insufficient (r2). Control: the same boss config without `workerMode` still auto-injects `team`/`schedule`/`team-roster`. Negative-verify: revert the worker-mode gate ⇒ the built-set assertion fails (auto-injection leaks `team`/`schedule` through).
