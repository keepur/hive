...
- **T2 (high-water formula).** ⚠ The correction pin, written against R2 (the default recommendation). (a) Non-empty tail ⇒ `injectionHighWaterTs === max(tail ts)`. (b) **Empty tail, round 1 (no trigger ts) ⇒ `injectionHighWaterTs === coveredThroughTs`, and the subsequent `setMeetingMark` is called** — the case the carried formula got wrong. (c) Round 0 ⇒ maxed against `roundZeroTriggerTs`. *Negative-verify: drop `coveredThroughTs` from the max ⇒ (b) fails with `undefined` and no mark write.* **If the ruling goes F1 instead of R2, five assertions invert, not two — both T2(a) cases, T2(b), T2(c), and T5**: `injectionHighWaterTs` is always `undefined` in summary mode and `setMeetingMark` is never called — every summary-mode assertion currently pinning a defined high-water value or a `setMeetingMark` call must be rewritten to pin the absence. True F1 is `injectionHighWaterTs: undefined` (delete the whole property, not just the `coveredThroughTs` term — deleting only that term still leaves a defined value on round-0 and non-empty-tail turns, which is the withdrawn fourth option above, not F1).
