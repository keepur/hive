...
**Timeouts (constants, not config):** connect/session RPC `clientSessionTimeoutSeconds: 30`; per-tool-call `timeout: 600_000` ms (aligned with the Bash max, generous enough for code-task-class tools). The spike validates SDK defaults are actually overridden by these options.
...
- **Metrics:** `buildResult` takes `toolCalls`/`toolMs`/`toolSummary` from `bridge.stats` (`toolSummary` = `"name×count"` joined, `"none"` when zero) — replacing hardcoded zeros (`:240-242`). Best-effort token usage from the run result's usage surface if trivially extractable (else stays 0 — matrix note; not a goal-gate).
- **Abort:** existing `AbortController` (`:42-44, :122-125`) additionally feeds the bridge's `signal`. On abort: SDK run loop stops (already wired via `runOptions.signal`), builtin executor kills its process group (SIGTERM → SIGKILL after 5 s), in-flight MCP/in-process calls have no protocol-level cancel — their settled results are discarded by the already-rejected run; wrapper catches keep them from becoming unhandled rejections; `finally` close tears down stdio subprocesses. `wasAborted`/aborted-result behavior unchanged (`isAbortError` path).
...
Vitest beside source; `npm run check` green. The exception-containment and abort tests are this ticket's headline obligations (epic §D4, Gate 1 assumption (b) via T6).
