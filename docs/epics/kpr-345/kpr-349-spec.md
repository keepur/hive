...
- **Tool-honesty gate for the un-flipped pilots:** tool-dependent prompt sections (toolkit, skills, file-tier-memory guidance) render **only for providers whose adapter actually executes bridged tools** — a `TOOL_EXECUTING_PROVIDERS = {openai}` set beside the assembly; KPR-352/353 each add their provider when they flip. Gemini/codex meanwhile get persona + constitution + roster + hot-tier memory and *no* tool claims — a strict upgrade from pilot instructions that never advertises tools their adapters (still zero-tools by canon) can't deliver. **The gate reaches into the memory block too (G4 correction):** two verified sites embed tool-instruction text alongside memory content — the hot-tier trailer's "available via `memory_recall`. Use it to search…" (`memory-manager.ts:117-121`, appended whenever warm/cold records exist) and the legacy-path file-listing's "Read relevant files via the memory MCP server (`view`)" (`prefix-builder.ts:158-162`). Both are false claims on a gemini/codex turn (zero tools) and both are in scope for `!toolsExecutable` stripping — see §D5.
...
- G4: Non-tool-executing Lane B providers (gemini/codex, pre-352/353) get the same assembly **minus tool-dependent sections** — no false tool claims. This includes tool-instruction lines embedded *inside* the memory block (the `memory_recall` trailer, the legacy file-listing's "via the memory MCP server" line) — memory content stays, memory tool-claims go (§D3, §D5).
...
| `memorySections(memoryManager, agentId, toolsExecutable)` | `:146-165` | hot-tier first, legacy `memory.md` + file-listing fallback; **returns the rendered block(s) plus the raw hot-tier string** so assembly can populate `ProviderMemoryBundle` without a second Mongo read; **`toolsExecutable` gates two tool-instruction lines only** (hot-tier `memory_recall` trailer, legacy-fallback "via the memory MCP server (`view`)" line) — content lines are never gated (§D3 correction, §D5); the Claude lane always passes `toolsExecutable: true` so `buildPrefix` output is unchanged |
...
Layer order (G2), tool-dependent layers marked †(gated by `toolsExecutable`): soul → archetype card → systemPrompt → constitution → team summary → †toolkit (§D4) → †file-tier-memory guidance (iff a `memory` entry is in the inventory — same predicate spirit as `prefix-builder.ts:135`, keyed on the inventory rather than `coreServerNames`) → †skills section (§D6, iff index non-empty) → hot-tier/legacy memory (‡ two tool-instruction lines inside this section are separately gated by `toolsExecutable` — the section itself always renders when memory exists, §D3/§D5) → datetime trailer. Datetime goes last for the same prompt-cache reason as the Claude lane — provider-side prompt caching is prefix-based too.
...
**Ruling:** `const TOOL_EXECUTING_PROVIDERS: ReadonlySet<LaneBProviderId> = new Set(["openai"])` lives in `turn-assembly.ts` beside the assembly (the same one-line-per-provider growth pattern as `SESSION_SEMANTICS`); `toolsExecutable = TOOL_EXECUTING_PROVIDERS.has(provider)` feeds the builder. KPR-352/353 each add their provider in the same commit that flips their adapter — the set and the stub-flip are one review surface. Gated sections: toolkit, file-tier-memory guidance, skills section. **Not** gated: constitution, roster, hot-tier/legacy memory *content* — provider-independent context that upgrades gemini/codex turns immediately.

**Correction (round-1 review, G4):** the gate is not purely section-level — it reaches inside the memory block. `memorySections` (§D1, §D5) renders two tool-instruction lines that are *not* memory content: the hot-tier trailer ("…available via `memory_recall`. Use it to search…", sourced from `memory-manager.ts:117-121`) and the legacy-path file-listing's "Read relevant files via the memory MCP server (`view`)" line (`prefix-builder.ts:158-162`). Both name a tool the model cannot call when `!toolsExecutable`. `buildProviderInstructions` strips these two lines (not the surrounding content — memory records, pinned entries, and the file-listing paths themselves all still render) when the gate is closed; the Claude lane (`buildPrefix`) is unaffected — it always has tools, so it keeps both lines verbatim, and the golden fixtures (§D2) prove byte-identity there. `assembly.skillIndex`/`memory` are populated uniformly (contract fields describe *hive's* surface; the bridge only runs where an adapter constructs it, so a populated index is inert for zero-tools adapters — and 352/353 inherit working skills for free).
...
- **Tool-claim gating inside the memory block (§D3 correction, blocking round-1 finding):** `memorySections` takes `toolsExecutable` and strips two tool-instruction lines when it is false — the hot-tier trailer's "…available via `memory_recall`. Use it to search…" (`memory-manager.ts:117-121`, appended by `getHotTierPrompt` whenever `countNonHot` is non-zero) and the legacy fallback's "Read relevant files via the memory MCP server (`view`) before starting tasks…" (`prefix-builder.ts:158-162`, appended after the file-listing). Both name a tool (`memory_recall`, the memory MCP's `view`) the model cannot invoke on a zero-tools gemini/codex turn — leaving them in would be exactly the false claim G4/D3 forbid. What stays on a gated turn: the memory record content itself (pinned/warm/cold sections, the legacy `memory.md` body), the raw warm/cold *count*, and the file-listing paths — only the imperative sentence naming the tool is dropped. Claude lane and openai (post-flip, tool-executing) keep both lines verbatim; this is a Lane-B-`!toolsExecutable`-only branch, so the golden fixtures (§D2, Claude-only) are untouched by construction.
...
- **Naming mismatch note (advisory, round-1 review):** the hot-tier trailer names the tool as bare `memory_recall` (`memory-manager.ts:121`), but the tool is defined inside the `structured-memory` in-process server (`structured-memory-mcp-server.ts:213`) and both SDKs' MCP-tool-naming convention is `mcp__<server>__<tool>` — i.e. the model-visible name is `mcp__structured-memory__memory_recall`, not the bare string the trailer prints. This is a **pre-existing wart shared by both lanes** (the Claude lane has carried this exact wording without visible breakage, likely because Claude models are tolerant of approximate tool references), not something this child introduces — but un-gating this line for a tool-executing Lane-B provider (openai today, 352/353 later) is a good, low-cost point to fix it rather than propagate it further. Two options, left to implementation: (a) align the trailer wording to the fully-qualified bridged name when rendering for a tool-executing provider (a Lane-B-only wording branch, orthogonal to the §D3/§D5 executable-gate — could reasonably fix the Claude lane too, but that risks the byte-parity goldens and is out of scope here), or (b) leave it matrix-noted (child 10) as a known cross-lane imprecision. Either resolution is acceptable; the fixture/testing sketch should assert *which* choice was made rather than leave the mismatch silent.
...
**Seam note:** KPR-348's spec phrasing at `kpr-348-spec.md:181` — "KPR-349 stays a pure assembly-side child and never edits bridge internals" — is superseded by this child's canon-delegated ruling: the cap-priority fix is a confined, in-scope edit to `applyNameAndCapEdges` (one function, ~10 lines, §D7 above), not a boundary violation. The PR description for this change should cite this line (§D7), not 348's, as the governing statement.
...
- **Gemini/codex turn (pre-352/353):** instructions carry constitution/roster/memory but no toolkit/skills/guidance, and the memory block's `memory_recall`/"memory MCP server" tool-instruction lines are stripped (§D3/§D5 correction) — memory content itself still renders; `assembly.skillIndex` populated but inert (no bridge constructed).
...
- **T2 — Lane B composition (unit on `buildProviderInstructions`):** section order (soul…datetime-last regex); †sections present iff `toolsExecutable`; file-tier guidance iff memory entry in inventory; hot-tier vs legacy fallback; skills section iff index non-empty; no `WebFetch`/`Task`/delegate strings anywhere in a Lane B toolkit; datetime format identical to Claude trailer. **Memory tool-claim gate (blocking finding, §D3/§D5):** for a `toolsExecutable: false` assembly with warm/cold records present, assert the rendered instructions contain **zero** occurrences of `memory_recall` and zero occurrences of "memory MCP server" — i.e. no tool-name strings survive from the memory block — while the memory *content* (record lines, warm/cold count, file-listing paths) still renders; the mirror case (`toolsExecutable: true`) asserts both lines are present, pinning the Claude-lane-equivalent behavior on openai.
