...
5. **File-collision risk between the reordered children is negligible:** 350 will touch `openai-agents-adapter.ts` session fields and possibly `SESSION_SEMANTICS.openai`'s value; 353 touches the codex adapter, one set literal in `turn-assembly.ts`, one comment in `types.ts`, `agent-manager.ts` (history-store ctor injection, codex-branch adapter options, the §D4 handoff clear), `index.ts` (store construction beside `SessionStore`), and net-new files. The shared-file overlap is real but disjoint at hunk level: 353's manager edits sit in the ctor, the codex adapter branch, and the KPR-313 guard — none touching the openai adapter branch (`agent-manager.ts:523-529`) where 350's manager edits, if any, would land.

**The one artifact that encoded the old assumption — superseded here:** the `SESSION_SEMANTICS` doc comment at `types.ts:41` says "codex gains replay in KPR-350." That was a KPR-347-era projection written before the auth split was understood. This child corrects the comment (replay = KPR-353; the *value* `stateless-replay` is untouched, per canon) in the same PR — so KPR-350's spec drafter inherits a truthful baseline and doesn't double-build codex replay.
...
- **SSE consumer extension:** `applyResponsePayload` (`codex-subscription-adapter.ts:333-342`) additionally captures `response.output` from the `response.completed` payload into `state.outputItems` (the full item array — message, reasoning-with-`encrypted_content`, `function_call`). Text deltas keep streaming to `onStream` across all rounds (intermediate think-aloud text streams like the openai lane's). **Final-reply semantics:** the delivered `RunResult.text` is the *final* round's assistant text only (each round consumes into a fresh per-round stream state); earlier rounds' think-aloud reaches `onStream` — and persists into replay history as assistant items — but never `RunResult.text`.
...
- **Usage accumulation:** input/output/cache-read tokens **summed across rounds, keyed on the `response.completed` event only.** `applyResponsePayload` is also invoked for `response.created`/`response.in_progress` (`codex-subscription-adapter.ts:321-323`), so a blind assign→`+=` conversion would multi-count within a round — accumulate exactly once per completed response (id/interim fields keep their current overwrite behavior). `costUsd` stays 0 (subscription).
...
- **API:** `load(agentId, threadId, provider): Promise<unknown[]>` (flattened turn items, oldest-first; `[]` on miss **or on any Mongo failure** — warn-and-continue), `append(agentId, threadId, provider, items): Promise<void>` (push one turn record + trim; fail-soft void), `clear(agentId, threadId): Promise<void>` (all providers for the thread — provider-agnostic so the KPR-313 hook stays generic; **awaited** and catch-swallowed at the call site, §D4). Items are stored verbatim and opaque — hive never introspects reasoning content; `encrypted_content` is provider-opaque ciphertext by design (nothing readable enters Mongo beyond what `sessions`-class storage already holds).
...
- **Trim (constants, not config):** whole turns dropped oldest-first while `turns.length > 1 && JSON.stringify(turns).length > HISTORY_CHAR_BUDGET = 200_000` — the `turns.length > 1` guard means the newest turn is never trimmed away, so a single over-budget turn is kept whole (this is the rule; the trim-boundary edge case below is its consequence, not a special case) (~50k tokens replayed worst-case; re-billed against subscription quota each turn — documented matrix caveat, mitigated by the backend's prompt caching, whose hits the adapter already surfaces via `cacheReadTokens`). Whole-turn granularity keeps `function_call`/`function_call_output` pairs and their reasoning items intact — a split pair is a malformed replay.
...
- **The one addition:** the KPR-313 guard's handoff branch (`agent-manager.ts:713-722`) also runs `await this.turnHistory.clear(ctx.agentId, ctx.threadId)` — **awaited**, catch-swallowed. Awaiting is load-bearing: the very turn that trips the guard proceeds through `prepareSpawn` to the adapter's `load()`, so only a clear that resolves before the spawn continues orders the Mongo delete ahead of the read — a fire-and-forget clear would let the post-handoff codex turn replay the stale doc. An awaited fail-soft Mongo op inside the R7 window is established posture (the guard already `await`s `sessionStore.get()` at `agent-manager.ts:705`), and `clear` never throws (§D3) — no new throw surface. **Accepted residual:** if the swallowed clear *fails* (Mongo blip at the handoff instant), the stale doc survives until TTL — same-provider turns never re-trip the guard, so no later clear fires; logged at warn, accepted (fail-soft canon beats blocking the turn). Rationale: history validity is contiguous-same-provider by construction. After codex→claude, the thread's real context lives in claude turns the codex history never saw; on the claude→codex return the guard fires again and clears, so the codex turn starts fresh **with** the KPR-313 §3.4 handoff annotation (which `prepareSpawn` already prepends — provider-agnostic, nothing to add). A handoff turn never replays stale history except in the logged failed-clear residual above.
...
- `types.ts:41` comment correction per §R (value untouched).
...
| `agent-manager.ts` | inject `TurnHistoryStore`; pass `historyStore`/`agentId` in the codex branch (`:514-521`); awaited handoff-branch `clear` (catch-swallowed) (§D4) | guard logic/ordering (R7 window), `finalizeSpawnResult` persistence rules, breaker acquire/record, `resolveProviderModel` |
...
- **Provider round trip codex→claude→codex:** both transitions trip the guard; history cleared (awaited) both times; the second codex era starts fresh with the handoff annotation (§D4).
- **Handoff clear fails (Mongo blip):** warn logged, turn proceeds — it may replay the stale doc (the sole exception to §D4's ordering guarantee), and the doc survives until TTL since same-provider turns never re-trip the guard. Accepted residual; a resulting 4xx lands in the §D7 self-heal.
...
- **T4 — KPR-313 interplay:** manager-level — handoff branch **awaits** `clear`, and the test pins ORDERING, not mere invocation: a deferred (manually-resolved) `clear` promise must resolve before the adapter's `load()` can observe the doc — i.e. `load()` is not reached while `clear` is pending; rejected clear doesn't throw in the guard window (catch-swallow); adopt branch doesn't call it; sessions row still written with `sessionId: ""` provider `codex` (existing pins stay green).
...
- **T6 — telemetry:** nonzero `stats.toolMs` ⇒ `llmMs = durationMs − toolMs` (clamped); usage summed across two mocked rounds whose SSE includes `response.in_progress` payloads carrying usage — completed-only accumulation, no multi-count (§D2); `toolSummary` rendering.
