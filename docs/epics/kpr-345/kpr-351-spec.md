...
**Child 6 of KPR-345** (two-lane provider-agnostic runtime). Epic spec: [kpr-345-spec.md](./kpr-345-spec.md) — the child-6 decomposition row carries this child's charter (reassignment via admin surface, validation checklist across the parity dimensions, inverse transition, spike-notes evidence); epic §D6 (Lane B resume parity) carries the KPR-313 transition rule the inverse leg proves.
...
- **Blast radius bounded (§D2):** only Luna's definition is mutated (via the token-gated admin REST API, which records version history); the engine swap affects all four keepur agents for the window, mitigated by: epic tip CI-green + Claude-lane golden gates, a Hermi Claude-lane smoke turn as a hard gate before any validation, a low-traffic window, same-pass rollback, and a pre-pass `mongodump` as belt-and-braces. Known accepted regression: the epic tip is 3 commits behind main — 2 functional PRs (slack snippet-download fix #324, linear label mgmt #325) + the v0.10.1 release bump — absent for the window only.
...
2. Note: epic `package.json` reads **0.10.0** while keepur runs 0.10.1 — the deployed engine will *display* an older version. Identification is by content, not version string: post-swap, write `~/services/hive/keepur/.hive/BUILD_INFO` (`kpr-345 epic <sha> <date>`) and record the SHA in spike notes. The 3-commit main delta (2 functional PRs #324/#325 + the v0.10.1 release bump) is accepted for the window (Key Points); ⚠ if the driver rebases the epic branch onto main first (recommended, not required), re-pin and rebuild.
...
- **deploy.sh dies mid-run (crash/Ctrl-C between `bootout` and `bootstrap`):** auto-rollback only fires on health-check failure — an interrupted script leaves the service booted out, possibly with a partial `.hive.next`. Recovery is mechanical, not improvised: re-run the same deploy command (`fetch_engine` starts with `rm -rf .hive.next`), or `launchctl bootstrap` the untouched `.hive` to restore the pre-deploy engine. Named in the abort ladder so the executor never guesses.
