...
- **T1 — routing:** `kimi/m` → `{provider:"kimi", model:"m"}`; `deepseek/m` likewise; `kimi/m:high` → effort survives; `kimi/` → model `""`; `kimi/:high` → model `""` + effort; unknown prefix (`mystery/m`) → claude fallback unchanged; slashless `kimi` → claude (pre-existing, documented); `providerFor` maps a kimi-model agent → `"kimi"` (extend the KPR-307 block at `agent-manager.test.ts:2255`). **Negative-verify: these fail against pre-change `resolveProviderModel` (fallback yields claude).**
