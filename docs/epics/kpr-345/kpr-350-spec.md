...
**Epic-spec label note:** the epic phase table (`kpr-345-spec.md` §D6 row 5) still titles this child "OpenAI resume (Conversations)" — that label is superseded by this ruling (mechanism = chaining); recorded here so KPR-355's matrix pass doesn't trip on the stale title. No epic-spec edit in this child (register canon governs).
...
- **Retry prompt is unannotated:** the fresh retry re-runs the already-shaped prompt exactly as built — no handoff-style annotation is injected, so the context loss is silent to the agent. Deliberate, precedent-consistent with the auth-rebuild arm (same block, same posture); an annotation is ruled out here because the shaping seam has already run and re-entering it for a retry would widen this child's surface for a one-exchange gap.
- **No double-stream risk:** stale-resume errors fail at request validation, before the first token — a first attempt that errored this way emitted no output, so the retry cannot double-deliver. Same shape as the auth-rebuild arm.
