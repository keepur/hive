...
**Architecture:** Four task streams (A pre-window code, B pre-window setup + G0, C in-window runbook, D post-window consolidation) over two work phases with an explicit human-gate boundary. **Code phase (Stream A, pre-window, in-repo):** the whole code diff is three files — `openai-agents-adapter.ts` loses `buildAuthAttempts`/`runWithAuthFallback`/the three oauth options and gains `buildClient()` + a missing-key throw (R1); `agent-manager.ts`'s stale-heal arm gains one post-lock `sessionStore.get` re-read with adopt-or-fresh retry (R2); tests strengthen/extend in the two suites + one additive classification pin. **Live phase (Streams B–C):** the spec §D3 runbook executed verbatim — deploy.sh single-instance rsync-fallback deploy of the locally-bundled epic engine, admin-API-driven Luna mutations (every one with its inverse pre-recorded), matrix legs, inverse transition, rollback + exact restore. **Post-window (Stream D):** spike-notes consolidation, KPR-355 row facts, conditional R5.
...
Run: `git diff --name-only 6414066..HEAD -- src/ CLAUDE.md docs/` (base = the plan commit `6414066` — the pre-Stream-A tip of the `kpr-345` lineage this worktree detached from; every commit after it is Stream A work)
Expected exactly:
```
CLAUDE.md
...
```bash
export ADMIN_TOKEN="${ADMIN_TOKEN:-$(security find-generic-password -s "hive/keepur/ADMIN_API_TOKEN" -w)}"  # recover from Keychain if this is a fresh shell (Task 6 may have run days earlier)
