...
- **`:effort`-only suffix on Lane A** (e.g. `kimi/:high`): `splitProviderModel(":high")` hits the `colon <= 0` guard and returns model `":high"` with **no** effort — the vendor receives `":high"` as a model id (a 4xx bad-model, config-fault-shaped, breaker-safe). This is identical to the pre-existing `codex/:high` behavior; special-casing it is a deliberate non-goal.
