...
| `finalizeSpawnResult` `:2287-2311` | `:2747-2866` (copies the three tool fields at `:2854-2856`, `llmMs` at `:2853`); `synthesizeAbortedResult` `:2307-2326`; `runWarmTurn` `:1779-1848` (returns `finalizeSpawnResult(ctx, runResult, …)` at `:1813`) | COSMETIC (large shift from KPR-386–414 merges; semantics intact) |
...
| `voice` config block | `config.ts:535-557` (after 322 E1/E3 + 323 C4 keys); `resolveVoiceWarmPathConfig` at `:248-263` (JSDoc `:248-255`, `VoiceWarmPathConfig` `:256-258`, fn decl `:260-263`) is the C6 template | COSMETIC |
...
- ⚠ **Ack count per turn — one class FIXED, one class watched at T1.** (a) *Fixed (pre-PR review R1):* subagent/delegate-nested `tool_use` blocks (`parent_tool_use_id != null`, which the SDK forwards by default) are now **excluded from ack injection** on both spawn loops — a single `Task` delegation would otherwise have spoken one canned hold line per NESTED call, stacking several "One moment." lines over what the caller hears as one silent gap. The exclusion is ack-only: usage accounting (KPR-401) and tool timing/logging still process those messages. (b) *Watch item, NOT decided here:* the spec-blessed §4.1 case of the model itself making N≥2 sequential **top-level** tool calls in one turn still acks once per call and is shipping unchanged. **T1 operator task:** listen to a legitimately multi-tool turn and record at the T1 gate whether stacked "One moment. / Let me check that." sounds natural or wants a follow-up per-turn cap. Do not add a cap pre-pilot — record the observation at the gate.
