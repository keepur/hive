...
| T2→T3 | Spawn prep | `prepareSpawn` (voice carve-out — returns immediately, **no model-router call**: `agent-manager.ts:1092-1093`); fresh `AgentRunner`; `buildSystemPrompt` **skipped** (systemPromptOverride) | `agent-manager.ts:1041`, `agent-runner.ts:1537-1714` |
| T3→T4 | CLI boot | envelope assembly (**in-process MCP server construction**, skill-projection FS work, session-cwd mkdir) then `query()` spawns the claude CLI subprocess: node boot + CLI init | `agent-runner.ts:1790` |
...
**Where the T2/T3 boundary actually falls (delivery correction, pre-PR review round 3):** the shipped `queryStartedAt` stamp is taken on entry to the runner's send path, *before* envelope assembly, so **in-process MCP server construction and the rest of envelope assembly are measured inside `bootToInitMs` (T3→T5), not `spawnPrepMs` (T2→T3)**. `spawnPrepMs` therefore covers `prepareSpawn` + adapter/runner construction only. W1's attribution table (Task 11) reads the fields as shipped — the table row above is annotated to match.
...
1. **Open (turn 1 of a call).** `spawnTurn(ctx)` sees `ctx.channel === "voice"`, `ctx.kind !== "reflection"`, `config.voice.warmPath.enabled`, provider = claude (full gate in §4.7), and no existing lease for `threadKey` → opens a lease: acquires the spawn ticket via the existing `withSpawnTicket` HOF with a lambda that resolves only at lease release (the ticket therefore holds the per-thread lock and one budget slot for the call's duration — all three stop-checkpoints, saturation recording, and finally-cleanup fire exactly as today). Inside the lease: build the runner once (same constructor args as `createProviderAdapter`), open `query()` with `prompt` = an AsyncIterable input queue, `resume` = **the adapter-supplied `ctx.sessionId` candidate, retained only when compatible with the route at actual lease opening — never a fresh session-store re-read**, `systemPromptOverride` = the voice prompt built once at call start, `includePartialMessages: true`. The resume-source rule is load-bearing: after a warm-turn failure, the adapter's outer retry lands cold with `sessionId: undefined` + full transcript (§5); if a subsequent lease open re-read the store, it would resume the very session the retry just escaped and double-inject the transcript. KPR-467 refines the authority split: the adapter renders both `voicePrompt.latestUserMessage` and `voicePrompt.fullConversation` and supplies the stored candidate and provider; the manager selects against admitted continuity. An existing lease consumes the latest user message regardless of current registry/provider or store availability; a new lease validates the candidate against its opening route, and cold admission selects after its identity guard. A fresh session always receives the full transcript at this admission boundary, including a provider change after the adapter read. A retry-cleared handle remains cleared. Empty latest-message renders retain the existing full-transcript fallback. Turn 1's text (full-transcript or greet-branch render, unchanged from `conversation-prompt.ts`) is the first queued message. `ticket.attachAbort(() => lease.close())` — **abort keeps kill semantics** (`stopAgent` walks tickets and must actually stop the call).
...
- `interrupt()` returns a Promise: `abortThread` stays synchronous-boolean — it dispatches the interrupt fire-and-forget with an attached `.catch()` that logs and escalates to `lease.close()` (an interrupt that fails means the session may be wedged; closing it converts the situation into the standard cold-fallback path, §6). The boolean reports "a matching in-flight turn was severed", same as 322. Edge: the warm dispatch can fire with **no generation in flight** — 322's close-listener registers before the pre-spawn awaits, so a disconnect during `buildVoiceSystemPrompt`/`sessionStore.get` on turn N reaches `abortThread` while the lease is idle (the turn was never queued; `clientGone` suppresses it adapter-side). `interrupt()` on an idle streaming session is SDK-unspecified; if it rejects, the `.catch()` escalation closes the lease and the call degrades cold — safe but avoidable, so idle-session interrupt is a W2 in-run behavior check (§7).
...
Anchored to kpr-320 @ d074d5c (main @ W6) at spec time. **Task 0 re-pin 2026-08-21:** delivery HEAD `bbd9581` (W3 + KPR-322 T1–T3 merged). Epic kpr-309 (W3 — KPR-311 router→adapter seam unification, KPR-312 classifier v2, KPR-313 session-identity guards, KPR-314 sidecar LLM registry, KPR-338 fixed-tier agents) is on this HEAD. All rows COSMETIC or UNCHANGED — no demote.

1. **`spawnTurn` / `withSpawnTicket` internals** (`agent-manager.ts:886-1208`) — this spec adds a branch at the top of `spawnTurn` and a lease that occupies a ticket; HOF `withSpawnTicket<T>(ctx, fn)`, `SpawnTicket` five-field shape (`:341-347`), three stop-checkpoints, in-CS budget, breaker-acquire as lambda-first (`:893-904`) all intact. **COSMETIC** (lambda body grew; those three not reshaped).
2. **`TurnContext` / session keying / resume semantics** — `_id "{agentId}:{threadId}"` + 7-day TTL intact; persist outer-gate `if (result.sessionId && !result.aborted)` (`:1829`) intact; KPR-313 added optional `sessionProvider`/`sessionHandoff` and `StoredSessionRef`. **COSMETIC** (keying unchanged).
3. **Adapter error taxonomy** — `"Spawn budget exceeded"` string-match (`voice-adapter.ts:480`) and `isAuthError` regex (`:24-29`) — no typed budget error landed. **UNCHANGED taxonomy**.
4. **`AgentRunner.send` / abort surfaces** (`agent-runner.ts:1874`, `abort` `:2316-2323`) — prompt assembly still in `send()`; production `query()` still constructed there (`:1968`). Cut points re-derived in the plan Task 0 block. **COSMETIC**.
5. **SDK pin** — `@anthropic-ai/claude-agent-sdk` package `^0.2.141`, lockfile resolved `0.2.141`. All five claims hold (`query` prompt union, `interrupt()` streaming-only comment, `close()`, `SDKUserMessage`, `streamInput()`; `SDKSession` still `@alpha`). ⚠#5 **DECIDED: omit `maxTurns`/`maxBudgetUsd` on the warm envelope**.
6. **322 E2 landing shape** — `abortThread(agentId, threadId): boolean` (`agent-manager.ts:1944-1966`) byte-identical to 322 plan Task 3 Step 1; close-listener before first await (`voice-adapter.ts:293-306`); mock `abortThread: vi.fn()`; assertion `("mokie", "voice:call-e2")` (`voice-adapter.integration.test.ts:610`). **UNCHANGED signature**.
7. Re-confirmed 2026-08-21: **no KPR-208 artifacts** (`git log --all --grep=KPR-208` empty); no W3 warm-path rails on this HEAD (`src/` has no `warmPath` / `WarmVoiceSession` / `openVoiceStreamingSession` / `buildQueryEnvelope`).
...
- ⚠ **Per-turn usage/cost attribution in streaming mode** — `result` usage AND `duration_ms` assumed per-exchange, not cumulative. Blast radius is wider than telemetry: `WarmVoiceSession` derives `llmMs = durationMs − totalToolMs` from the per-exchange `result.duration_ms`, and that value is what `circuitBreakers.record(permit, …, llmMs)` feeds to KPR-306's **p95-latency trip arm** (240s). If `duration_ms` turns out cumulative across the streaming session, late-call turns would inflate the latency window and could open the **claude** circuit engine-wide — not merely skew a log field. Bounded today (flag defaults off); W2 is the gate that verifies it before any pilot rollout, and its behavior-check list carries an explicit per-turn `llmMs`/duration sanity check.
...
## 12. Sources (checked 2026-07-14; Task 0 re-pin 2026-08-21 @ `bbd9581`)

Hive code (lane worktree `/Users/mokie/github/lane-kpr-323-deliver` @ `bbd9581`): `src/agents/agent-manager.ts` (spawnTurn `:886-1092`, withSpawnTicket `:1110-1208`, reflection `:1318-1384`, prepareSpawn voice carve-out `:1625-1627`, finalize `:1827-1863`, abortThread `:1944-1966`, getSnapshot `:1252-1288`), `src/agents/agent-runner.ts` (send `:1874`, in-process MCP `buildInProcessServers` `:1391` merged at `:1892`, systemPromptOverride consumption `:1900`, query options `:1968-2023`, init/delta/result loop `:2073-2197`, abort `:2316-2323`), `src/agents/session-store.ts` (`_id` keying `:13`/`:94`, 7-day TTL `:56`), `src/channels/voice/voice-adapter.ts` (pre-spawn `:311-317`, onStream/SSE `:354-370`, outer retry `:429-443`, error rows `:458-496`, "Voice turn complete" `:528-537`, abort-on-disconnect `:293-306`), `src/channels/voice/conversation-prompt.ts`, `src/channels/dispatcher.ts` (routeVoiceTurn `:752`, UNCHANGED), `src/agents/prompt-builder.ts` (buildVoiceSystemPrompt `:12+`), `src/config.ts` (voice block `:473-492`). SDK: `@anthropic-ai/claude-agent-sdk` ^0.2.141 typings (`sdk.d.ts` — `Query.interrupt()`/`streamInput()`/`close()`, `query({prompt: string | AsyncIterable<SDKUserMessage>})`, `SDKSession` @alpha). History: pre-KPR-220 long-lived per-agent `query()` loop (CLAUDE.md §Spawn coordinator; KPR-210 eb064c6, KPR-220 4b62b89-era) — precedent for streaming-input sessions in this engine; `git log --all --grep=KPR-208` → empty (subprocess pool never shipped). Siblings: `docs/epics/kpr-320/kpr-322-spec.md` (§4–§9, §13–§15, §18), `kpr-322-plan.md` (Task 3 E2 wiring, Testing Contract).

### KPR-467 corrective review clarification

The call-pinned timeout also supplies breaker `acquire` metadata (`deadlineMs`), so a legitimate long half-open probe stays valid for its watchdog deadline plus the breaker's existing grace. Prompt selection changes are limited to the adapter/manager admission boundary and warm lease reuse; existing cold inner auth/stale-handle retry semantics are unchanged. The adapter's outer retry still clears the handle and provider and carries the full transcript. Transcript-bearing `voicePrompt` is never added to logs.
