...
- **Telemetry (322 lever c) is supporting, not masking (S4).** `TurnResult` already carries `toolCalls` / `toolMs` / `toolSummary`; the voice adapter's "Voice turn complete" line does not yet log them. Additive fields + an ack-injection count. No new protocol.
...
- **In scope:** ack helper + both spawn loops, voice-prompt supplement, adapter telemetry, `orders` contract, voice-pilot fixture, T-gates (designed-not-run). **Out of scope:** 325 personas/rubric, 321 Twilio ops, redoing 322/323, `/v1/tools`, LiveKit function tools, worker `session.say` filler, mid-tool "still looking" loops, Lane B voice ack, production `orders` implementation.
...
| **S4** | **Log `toolCalls` / `toolMs` / `toolSummary` / `toolAckInjected` on "Voice turn complete".** No new collection. | Lever (c) as observability so T-gates and 325 can attribute pauses. Fields already exist on `TurnResult` (`agent-manager.ts:146-148`); the adapter omits them today (`voice-adapter.ts:532-550`). | New `kind=voice_tool_stats` heartbeat — premature; per-turn line is enough |
...
|---|---|---|
| `HiveLLM.chat()` | Accepts 1.6.4 `toolCtx` / `toolChoice`; **does not send `tools` in the POST** (`hive-llm.ts:143-154`) | Keep ignoring `toolCtx`. Never add LiveKit function tools to `new voice.Agent({ instructions })` (`session.ts:201-206`) |
| `HiveLLMStream` | Yields content SSE only; records `maxInterChunkGapMs` between chunks | Unchanged. An injected ack is just another content chunk — the gap metric should fall on tool-turns if S2 works |
...
| `TurnResult.toolMs` / `toolCalls` | Computed; dispatcher logs them on chat channels; **voice adapter does not** | S4 |
...
on assistant message:
    ...
    for each tool_use block:
        if config.voice.toolAck.enabled && !streamedThisSegment && onStream:
...
- Two silent tools back-to-back → ack at each boundary (second one may be a different phrase). Acceptable on a phone; better than a second dead gap.
...
Phone-native, no "as an AI", no markdown. Rotate in order, wrap. Delivery may retune wording without a spec amendment; 325 may later persona-split (⚠ non-blocking).
...
- `shouldInjectToolAck(args: { enabled: boolean; streamedThisSegment: boolean; hasOnStream: boolean }): boolean`
- `nextAckPhrase(state: { index: number }): { phrase: string; index: number }` — per-call or per-process rotation is fine; per-turn is enough
- No I/O, no logging of the phrase (content). Callers log `toolAckInjected` as a count only

Wire:

1. `AgentRunner.send` — in the `assistant` / `tool_use` branch, gated on `onStream` (voice always passes one when streaming; chat channels that pass `onStream` get the same ack if they ever stream mid-tool, which is fine and tiny).
2. `WarmVoiceSession.consumeOneTurn` — identical observation. Warm eligibility unchanged (323 §4.7).
...
| `toolAckInjected` | count from the spawn loop (0 if disabled or unused) |
...
| `SERVER_CATALOG` blurb | "Purchase-order lookup — status, dates, line remainders, by PO number or vendor" / Use for: grounding a live vendor or ops call / Not for: creating or changing orders (read-only); contact identity (use `contacts`) |
...
No `src/orders/` production server. No Mongo collection. No `hive.yaml` ERP URL. No Honeypot key. Those land with W1B against **this section as the freeze**. A W1B spec that changes tool names or adds write tools is a D2 break and needs an epic-register amendment.
...
| Entitlement | `coreServers: ["voice-fixture"]` on the **`voice-pilot` test agent only** (322 §14.2 ⚠ — never Nora/Sige) |
...
| # | Change | Where | Size |
|---|---|---|---|
| C1 | `voice-tool-ack.ts` helper + unit tests | `src/agents/` | small |
| C2 | Inject at `tool_use` in `AgentRunner.send` | `agent-runner.ts` | small |
| C3 | Same inject in `WarmVoiceSession.consumeOneTurn` | `warm-voice-session.ts` | small |
| C4 | Voice Call Mode paragraph | `prompt-builder.ts` | trivial |
| C5 | "Voice turn complete" fields (S4) | `voice-adapter.ts` | trivial |
| C6 | `voice.toolAck.enabled` liberal loader (absent → on; literal `false` → off) | `src/config.ts` | trivial |
| C7 | `voice-fixture` in-process MCP + strip-if-not-pilot | `src/voice/` or `src/agents/` + runner wiring + catalog | small |
| C8 | `SERVER_CATALOG` stub entry for `orders` (blurb only, no server) + comments pointing at this spec §6 | `src/tools/server-catalog.ts` | trivial |

**No worker diff. No new HTTP route. No LiveKit Agent tools. No schema/migration. No new secrets.**

Rollback: `voice.toolAck.enabled: false` (silence during tools, pre-324) + remove `voice-fixture` from `voice-pilot`. Leave C4/C5 in place (prompt + telemetry are harmless).
...
Phrases, rotation, delay cap, segment rule: **constants** in `voice-tool-ack.ts` / fixture module (323 timeout pattern).
...
| 322 §14.2 script | Already "order status + one lookup pause + two barge-in points". T-gates reuse that script on `voice-pilot` with `voice-fixture` as the pause |
| 325 | Consumes masking + (once W1B unparks) `orders` on Nora/Sige. Personas/rubric still 325. May retune ack phrases |
| W1B / KPR-300 | Implements §6 exactly. Park does not block 324 delivery or 325's non-order pilot turns |
...
| Non-streaming voice request | No `onStream` → no inject (degenerate; LiveKit/Vapi stream) |
...
| Lane A cold voice (kimi/…) | C2 runs (same `AgentRunner.send`). Warm lease stays Claude-only (323) |
...
| Model calls `orders_*` before W1B | Tool absent unless W1B shipped; model should not see the schema. Do not register a failing stub on Nora |
...
1. `shouldInjectToolAck` true only when enabled + not streamed + has onStream.
2. Text-then-tool: no inject; silent-tool: inject once; two silent tools: inject twice; text+tool same assistant message: no inject.
...
4. Cold and warm loops both honor (3) — mock `onStream`, emit a `tool_use` assistant message with no prior deltas.
5. Voice adapter "Voice turn complete" includes `toolCalls` / `toolMs` / `toolAckInjected` from `TurnResult` (no phrase text).
...
Reuse 322 `voice-pilot` + 10-turn vendor-style script. Prefer the **Vapi path or LiveKit path, whichever is live** — masking is behind the HTTP seam so either is valid. Warm-on for T2.
...
- **Production `orders` implementation**, ERP selection, migrations, Honeypot keys for an order API.
...
- ⚠ **Test agent `voice-pilot` still exists** as 322's dedicated def — 324 must not point the fixture at Nora/Sige. Confirm at delivery Task-0.
...
Hive code (lane worktree `/Users/mokie/github/lane-kpr-324-mature` @ `f0f4abd`): `src/voice-worker/hive-llm.ts` (text-only POST, `maxInterChunkGapMs`), `src/voice-worker/session.ts` (`session.say` fallback-only; `Agent` without tools), `src/voice-worker/telemetry.ts`, `src/channels/voice/voice-adapter.ts` (SSE text, E2, "Voice turn complete" without tool fields), `src/channels/voice/conversation-prompt.ts` (Phase-2 skip), `src/channels/voice/openai-translator.ts` (`formatSSEToolCallChunk` unused), `src/agents/warm-voice-session.ts` (`tool_use` timing + interrupt), `src/agents/agent-manager.ts` (`TurnResult.toolCalls`/`toolMs`, `abortThread`), `src/agents/agent-runner.ts` (`text_delta` filter, `tool_use` start), `src/agents/prompt-builder.ts` (`buildVoiceSystemPrompt`), `src/agents/in-process-servers.ts`, `src/contacts/contacts-mcp-server.ts`, `src/tools/server-catalog.ts`, `src/voice/voice-mcp-server.ts` + `src/voice/livekit-voice-mcp-server.ts` (E4 initiation). Grep: no `/v1/tools` route; no purchase-order/ERP modules.
