...
| Voice | `src/channels/voice/voice-adapter.ts` — thrown spawnTurn errors → catch → 503 `endWithError` paths (auth/budget have dedicated 503s) | Generic 503 to Vapi |
...
**2g. Replay failure handling:** replay attempt fast-fails (`ProviderCircuitOpenError` again) → item back to `pending`, attempt counter NOT incremented (breaker-open retries are free and expected — counting them would exhaust attempts during the outage itself). Replay runs and fails with a real error → attempts+1; after `maxReplayAttempts` (default **3** ⚠) mark `failed` — the normal dispatcher error path already delivered `"Something went wrong: …"` to the thread on each real attempt, so the user is not left in silence.
...
⚠ Prefix-based detection is stable at baseline (all synthesized ids are format-fixed in `scheduler.ts`) but stringly; the implementer may instead add one `meta.outagePolicy` field at the three synthesis sites if review prefers — behavior identical.
...
The second classification leg needs the agent's provider in the dispatcher: expose `agentManager.providerFor(agentId)` (one-liner over `resolveProviderModel(config.model).provider`, both already in `agent-manager.ts` per KPR-306 §"wrap point") — additive, no contract change. **Post-turn success on a provider clears its episode** (episode tracker hook in the success path — one line).
...
3. Re-dispatch: `dispatcher.dispatch({ ...doc.workItem, id: "replay:" + doc.attempts + ":" + doc.itemId, text: replayWrap(doc), meta: { ...doc.workItem.meta, targetAgentId: doc.agentId, outageReplay: true } })`.
   - Fresh id defeats the 60s dedup map across attempts; `targetAgentId` pins agent resolution (step 0); original `source`/`meta.slackThreadTs` route the reply to the original thread through the normal delivery path.
4. Outcome: `dispatch` resolves without the outage path re-triggering → `done`. Fast-failed again (the catch re-enqueues by `itemId` — unique index makes this an update) → back to `pending`, attempts unchanged, **no second notice** (episode dedup holds). Real failure → attempts+1, `failed` at `maxReplayAttempts`.
5. On success of an item, loop to the next immediately (drain), yielding between items; stop draining on the first fast-fail.

The tick needs no breaker-state check: attempt-and-see is free while open (pre-router fast-fail) and IS the probe when the cooldown has elapsed. One subtlety the plan must handle: re-entrancy — the replay item's own fast-fail routes through §7.2, which must recognize `meta.outageReplay` and update the existing doc rather than create a duplicate or send a notice (the `itemId` upsert + `noticeSent` flag cover both).
...
| `src/channels/dispatcher.ts` | extract shared failure helper from `:258-275` + `:646-663`; outage classification (instanceof + post-turn open check incl. the success-branch error guard near `:207-226`); episode-clear on success; recognize `meta.outageReplay` |
...
Untouched by design: `src/sweeper/retry-queue.ts`, `src/scheduler/scheduler.ts` (unless the operator picks the `meta.outagePolicy` variant in §5-3a), all adapter `deliver()` implementations, KPR-306's breaker files.
...
- ⚠ Prefix-based source policy detection (§5-3a) vs. a `meta.outagePolicy` field — behaviorally identical; implementer/reviewer's choice.
...
- `dispatcher.test.ts` (extend) — instanceof path queues + delivers plain-text notice (assert `error` UNSET on the delivered WorkResult — the SMS-skip regression guard); post-turn open-state path (errored TurnResult + open snapshot → outage path; + closed snapshot → legacy error path; + shadow `enabled:false` open snapshot → legacy path); skip for `sched:`; silent-queue for `callback:`/`event:`/`team-`; overflow variant at maxDepth; `outageReplay` re-entrancy (no dup doc, no second notice); episode cleared on success; fan-out single notice per thread.
- `outage-replay-processor.test.ts` — drain order; fast-fail → pending, attempts unchanged, drain stops; real failure → attempts+1 → `failed` at cap; replay id format defeats dedup (dispatch twice within 60s with distinct ids); expiry → batched notice per thread; wrap text for notify vs silent policies; `enabled:false` no-ops.
