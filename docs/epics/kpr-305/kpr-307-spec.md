...
- **The probe-failure turn is also made honest — including hang-type timeouts.** A half-open probe that fails does NOT throw `ProviderCircuitOpenError` — it runs the real turn and fails with the raw provider error in `TurnResult.errors[0]` (or, for a runner-deadline timeout, with `TurnResult.timedOut` set and `errors` typically empty, since the abort path returns before any provider error string is captured), then the breaker re-opens. Rule: after a turn completes, if `agentManager.circuitBreakers.stateFor(provider)?.state === "open"` AND (the result classifies into `HARD_FAULT_KINDS` OR `timedOut && aborted`, §7.2), treat it exactly like a fast-fail (queue + notice) instead of delivering `"Something went wrong: …"` or, in the timeout case, a bare `"_No response._"`. This also covers the trip-crossing turn (the Nth consecutive fault that opened the breaker) and, since the additive `TurnResult.timedOut` flag (Finding 3, review round 2) closes the gap where `errors` alone never populated, every trip-crossing and half-open-probe turn during a hang-type outage as well.
...
export interface OutageQueueDoc {
  _id: ObjectId;
  itemId: string;            // original WorkItem.id — composite-unique with agentId (idempotent enqueue per fanned agent, Finding 4 review round 2)
...
Store API: `enqueue(doc)` (upsert on the composite `(itemId, agentId)` key — double-enqueue for the same agent is a no-op, but a fan-out dispatch enqueues one doc per fanned agent), `claimNext()` (atomic `findOneAndUpdate` pending→replaying, oldest `enqueuedAt` first — copies the callback poller's mark-before-dispatch pattern at `scheduler.ts:265-269`), `release(itemId, agentId, outcome)` (keyed the same composite way — Finding 4, review round 2), `pendingCount()`, `expireOlderThan(cutoff)` (returns expired docs grouped by thread for the batched notice). Indexes: `{ itemId: 1, agentId: 1 } unique` (composite — a unique index on `itemId` alone would collapse a fan-out dispatch's N agents to one queued doc, silently dropping N−1 of the fanned agents' replies; Finding 4, review round 2), `{ status: 1, enqueuedAt: 1 }`, TTL index on terminal-status docs (`doneAt`, 7d) for hygiene. Boot recovery: `replaying` docs older than one turn deadline (300s + slack) revert to `pending` (crash between claim and release).
...
classify:
  ...
  else if turn completed without normal delivery AND stateFor(providerFor(agentId))?.state === "open"
       AND ( (classifyTurnResult({ error: runResult.error, timedOut: turn.timedOut, aborted: turn.aborted }).outcome === "fault"
              AND HARD_FAULT_KINDS.has(classification.kind))
             OR (turn.timedOut === true AND turn.aborted === true) )   // Finding 3, review round 2 — hang-type timeout coverage
                                                                → outage path (probe-failure / trip-crossing turn)
  else if item.meta?.outageReplay                               → legacy path, but first store.release(itemId, agentId, "pending")   // Finding 2, review round 2 — see below; attempts unchanged, then deliver/continue as today
...
outage path:
  ...
  if item.meta?.outageReplay
       → store.release(itemId, agentId, outcome); return   // §5-2g table — dispatcher writes the outcome; runs BEFORE the depth-cap check below (Finding 1, review round 2) — depth is irrelevant to an item that already holds a queue slot, so a replayed fast-fail at maxDepth must resolve the existing doc, not take the overflow branch and strand it in `replaying`
...
- KPR-306's exported classifier (`classifyTurnResult`, `HARD_FAULT_KINDS` — `src/agents/provider-adapters/error-classification.ts`) — plus one additive field this ticket requires on `TurnResult`: **`timedOut?: boolean`, propagated from `RunResult.timedOut`** (KPR-306 adds this flag to `RunResult` and explicitly permits additive downstream evolution — `kpr-306-spec.md` §"Timeout needs one runner change" / "Frozen fields"). `TurnResult.aborted?: boolean` (mirroring the already-existing `RunResult.aborted`) needs the same threading — the dispatcher's own `convertTurnResult` today hardcodes `aborted: false` with a comment claiming it's "never read downstream" (`dispatcher.ts:321-325`); that claim stops being true once this leg reads it, so the hardcode is replaced with `turn.aborted` as part of this same small touch. The dispatcher's call becomes `classifyTurnResult({ error: runResult.error, timedOut: turn.timedOut, aborted: turn.aborted })`. **Gate (Finding 3, review round 2 — DECISION MADE, reviewer's option (a)):** a turn takes the outage path when EITHER `classifyTurnResult(...).outcome === "fault"` AND `HARD_FAULT_KINDS.has(classification.kind)`, OR `turn.timedOut === true AND turn.aborted === true` (mirroring KPR-306's own timeout classification rule, §"Classification order" rule 1). The second disjunct is required because a runner-deadline timeout (`RunResult.timedOut`) typically leaves `error` unset — the abort path returns before a provider error string is captured — so `TurnResult.errors`, which builds only from `result.error` (`agent-manager.ts:1193`), never populates; without the disjunct the `classifyTurnResult`-only leg silently never fires for a hang-type outage, and every trip-crossing turn and every half-open probe turn delivers `"_No response._"`, unqueued and unnoticed. Only a result that classifies into `HARD_FAULT_KINDS` OR trips the timeout disjunct takes the outage path; a `non-provider` classification with no timeout, while the breaker happens to be open, follows the **legacy** "Something went wrong" path unchanged — it isn't a provider fault and its side effects should not be silently re-run.

**Thrown-error legacy branch must resolve replay docs too** (Finding 2, review round 2). The classify `else` branch — a thrown error that is neither `ProviderCircuitOpenError` nor a breaker-open hard/timeout fault — today does nothing outage-specific: the existing catch (`dispatcher.ts:258-275`/`:646-663`) delivers `"Something went wrong: ${err}"` and returns. For a *replay* attempt (`item.meta?.outageReplay`) that throws such a legacy error — e.g. `spawnTurn`'s pre-existing "Spawn budget exceeded" throw with the breaker closed — this branch releases nothing, so the `outage_queue` doc is left in `replaying` forever (the outage-path branch above is the only writer of `release`). Fix: the legacy branch, when `item.meta?.outageReplay` is set, calls `store.release(itemId, agentId, "pending")` (attempts unchanged) before delivering the existing error text — budget-exceeded and its kin (lock contention, transient spawn-coordinator saturation) are transient resource contention, not a provider verdict on the turn's content, so they get the same treatment as a fast-fail-again (§5-2g), not a real-failure attempts+1. Non-replay turns through this branch are completely unchanged.
...
**Disabled-agent drop and unresolvable/deleted pinned-agent drop are two early-returns the outcome table must cover** (Finding 2, review round 1; extended, Finding 6, review round 2). Two returns precede the try/catch: `resolvedList.length === 0` (`dispatcher.ts:139-146` — `resolveAgents` found no match at all, e.g. the replay's pinned `targetAgentId` names an agent that was deleted or is otherwise unresolvable) and `activeList.length === 0` (`dispatcher.ts:149-157` — resolution matched but the agent is disabled). Either way, a replayed item (`meta.outageReplay`) whose target agent is gone mid-outage would otherwise never resolve its `outage_queue` doc. Add the same check at BOTH returns: if `item.meta?.outageReplay`, call `store.release(itemId, item.meta.targetAgentId!, "expired", "agent disabled/deleted — will not be replayed")` before returning (§5-2g table) — with **no fall-through resolution**: a deleted or disabled pinned agent must terminate the replay as `expired`, not silently re-route to some other agent `resolveAgents` might otherwise match. The replayed answer belongs to the agent the user originally reached, not a substitute.
...
- **Fan-out threads:** each fanned agent fast-fails separately; notice dedup is per-thread, so one notice despite N agents (the notice speaks as the first-failing agent — acceptable; a "system" voice variant is a wording option for the operator). Each fanned agent's queued item is its own `outage_queue` doc, keyed `(itemId, agentId)` (Finding 4, review round 2) — a unique index on `itemId` alone would collapse N fanned agents to one queued doc (first enqueue wins, N−1 silent no-ops, only 1 of N answers ever replayed); the composite key, matching `(itemId, agentId)`-keyed release, and per-doc targeted redispatch (§7.4 step 3) ensure all N agents' answers replay independently.
...
| File | Change |
|---|---|
| `src/outage/outage-queue-store.ts` | **new** — collection schema, enqueue/claim/release/expire; composite `(itemId, agentId)` unique key so a fan-out dispatch queues N independent docs instead of collapsing to one (Finding 4, review round 2) |
...
| `src/channels/dispatcher.ts` | extract shared failure helper from `:258-275` + `:646-663`; outage classification (instanceof + post-turn open check gated by KPR-306's `classifyTurnResult`/`HARD_FAULT_KINDS` OR `timedOut && aborted`, incl. the success-branch error guard near `:207-226`; Finding 3, review round 2); `convertTurnResult`'s `aborted` mapping switches from a hardcoded `false` to `turn.aborted` now that it's read (Finding 3, review round 2); episode-clear on success gated on `stateFor(provider)?.state !== "open"`; dedup-check bypass for `item.meta?.outageReplay` (`:104-109`); BOTH early returns (`resolvedList.length === 0` at `:139-146` and disabled-agent `activeList.length === 0` at `:149-157`) resolve `outage_queue` to `expired` for replay items with no fall-through resolution (Finding 6, review round 2); legacy thrown-error branch resolves replay docs to `pending`, attempts unchanged, instead of stranding them in `replaying` (Finding 2, review round 2); release-before-depth-cap ordering in the outage path so a replayed fast-fail at maxDepth still resolves the doc (Finding 1, review round 2); dispatcher writes the replay outcome directly, keyed `(itemId, agentId)` (§5-2g table, Finding 4 review round 2), instead of the poller inferring it; plain-text terminal-failure notice on `failed` for notify-policy channels |
| `src/agents/agent-manager.ts` | additive `providerFor(agentId)` helper (wraps existing `resolveProviderModel`); additive `TurnResult.timedOut?: boolean` (propagated from `RunResult.timedOut`, KPR-306) and `TurnResult.aborted?: boolean` (propagated from the already-existing `RunResult.aborted`), consumed by the dispatcher's classify leg gate on `timedOut && aborted` (Finding 3, review round 2) |
...
- `outage-queue-store.test.ts` — enqueue idempotency pinned to the composite `(itemId, agentId)` unique key (double-enqueue for the same agent is a no-op; a fan-out dispatch produces N independent docs, one per fanned agent — not collapsed to one per itemId, per §7.1/§7.4), claim ordering (oldest first), atomic claim (no double-claim), expiry grouping by thread, boot-recovery of stale `replaying`.
...
- `dispatcher.test.ts` (extend) — instanceof path queues + delivers plain-text notice (assert `error` UNSET on the delivered WorkResult — the SMS-skip regression guard); post-turn open-state path (errored TurnResult + open snapshot + `HARD_FAULT_KINDS` classification → outage path; + `non-provider` classification while open → legacy error path, Finding 4; + closed snapshot → legacy error path; + shadow `enabled:false` open snapshot → legacy path); **timeout gate** — `timedOut === true && aborted === true` with the breaker open → outage path even when `errors` is empty (the hang case the round-2 fix targets); `timedOut === true` with the breaker closed → legacy path, unqueued; skip for `sched:`; silent-queue for `callback:`/`event:`/`team-`; overflow variant at maxDepth; **release-before-depth** — a replayed item (`meta.outageReplay`) that fast-fails while the queue is already at `maxDepth` resolves its existing doc via `store.release(...)` and does NOT take the overflow branch (Finding 1, review round 2); `outageReplay` re-entrancy (no dup doc, no second notice, dedup bypass lets the same id redispatch, Finding 1); **legacy thrown-error branch** — a replay item that throws a non-outage error (e.g. spawn-budget-exceeded) with the breaker closed releases the doc back to `pending` with attempts unchanged, rather than stranding it in `replaying` (Finding 2, review round 2); disabled-agent replay (`activeList.length === 0`) resolves to `expired` (Finding 2); **unresolvable/deleted pinned-agent replay** (`resolvedList.length === 0`) also resolves to `expired`, same as the disabled-agent case, with no fall-through resolution to a substitute agent (Finding 6, review round 2); episode cleared on success only when `stateFor(provider)?.state !== "open"` (Finding 3); fan-out single notice per thread via synchronous test-and-set (Finding 8); `failed` transition delivers a plain-text terminal notice on notify-policy channels, none on silent (Finding 6).
...
- `voice-adapter.test.ts` (extend) — `ProviderCircuitOpenError` → spoken outage completion, not generic 500.
...
Epic KPR-305 canon at draft time: Gate 1 rulings **D3** (KPR-308 LAN architecture — first canon), **D4** (doctor sections informational; identity-class only may fail — §7.6 complies), **D5** (KPR-306 scope), **D6** (audit artifact unrecoverable). Nothing here revisits them. KPR-306's **Open-Circuit Contract** is consumed as-frozen (fields used: `provider`, `openedAt` — with the episode caveat, `retryAfterMs` — deliberately NOT quoted to users, plus `stateFor()`/`snapshot.state`/`snapshot.enabled`); the additive asks on the hive side are `agentManager.providerFor()` and `TurnResult.timedOut?`/`TurnResult.aborted?` (propagated from `RunResult`), all KPR-307 code, not a contract change. KPR-307 additionally binds to `classifyTurnResult(input: TurnFaultInput)` and `HARD_FAULT_KINDS` as exported names/signatures from KPR-306's error-classification module (`src/agents/provider-adapters/error-classification.ts`) — promised in 306's design (§"Error classification") — and this freeze (Finding 8, review round 2) is recorded in the epic decision register so KPR-306's implementer knows a sibling ticket binds to them (register entry posted by the epic driver, not this spec). The operator's rulings on §5 Decisions 1–3 become new canon entries for this epic.
