...
4. `error` matched against a pattern table (case-insensitive regex, exact patterns are an implementation detail but must cover at least):
   ...
   - `auth`: `\b401\b | \b403\b | authentication | unauthorized | invalid.?api.?key | OAuth session is not available | not.?authenticated | credentials\.json | ANTHROPIC_API_KEY | authToken | resolve authentication` — **this row MUST be a superset of every `isAuthRebuildResumeError` alternate** (`agent-manager.ts:185-189`: `resolve authentication | credentials\.json | not authenticated | 401 Unauthorized | ANTHROPIC_API_KEY | authToken`); a sentinel the auth row doesn't cover would classify `non-provider` and **reset** the hard-fault streak (rule 5's reset-on-non-provider semantics), so a persistent auth outage would never trip. Any future addition to the sentinel list must extend this row in the same change.
...
5. Default → `fault: non-provider`. **Fail-safe bias: an unrecognized error string must never trip the breaker.** Under reset semantics (rule "Trip rules" below: a `non-provider` fault resets `consecutiveHardFaults`), a false negative (missed provider fault) doesn't just delay a trip by one turn — it resets the streak, so the trip is delayed by a full fresh run of hard faults, and if unrecognized strings keep interleaving with real faults the breaker may never trip at all. False positives (tool failure tripping the breaker) take a healthy provider offline outright — the asymmetry (a stuck-open false trip vs. a merely-slower true trip) still dictates the default toward `non-provider`.
...
In `AgentRunner.send()`: a local `let timedOut = false;` set inside the existing deadline callback (`agent-runner.ts:1812-1818`), but **only when the abort actually cancels an active query** — mirror `abort()`'s own guard (`agent-runner.ts:2028-2035`: it no-ops when `this.activeQuery` is `null`). Concretely: `if (this.activeQuery) { timedOut = true; } this.abort();` (or check `activeQuery` before calling `abort()`). The scenario this guards against is **operator-abort-then-late-deadline**, not a result-tail race: `clearTimeout(deadline)` and `this.activeQuery = null` run back-to-back, synchronously, in the same `finally` block (`agent-runner.ts:1962-1963`), so there is no window where a result has landed but `activeQuery` is still non-null pending that `finally` — the deadline timer is guaranteed cleared in the same tick `activeQuery` is nulled from that path. The real null-`activeQuery` case is: the operator (or a stop-agent sweep) calls `abort()` directly (`agent-runner.ts:2028-2035`), which sets `this.activeQuery = null` and `_aborted = true` immediately, *before* the in-flight `try` block (and its `finally`) has unwound and cleared the deadline timer. If the deadline then fires in that gap, an unguarded callback would still see the timer pending and stamp `timedOut = true` onto what is really an operator abort, misclassifying it as a timeout fault instead of a neutral abort. The guard (`if (this.activeQuery)`) closes that gap: by the time `abort()` has run, `activeQuery` is already `null`, so the later deadline fire correctly no-ops. (Converse residual, accepted: if the deadline fires in the narrow tail window where a result has just landed but `activeQuery` is *still* set — i.e. before the `finally` block runs — the turn is marked both `timedOut` and `aborted` even though it effectively completed; at a 300s deadline this window is negligibly small and the mislabel is a completed-but-unreturned turn, not a false trip on a healthy provider.) New optional field on the returned object / `RunResult` interface (`agent-runner.ts:120-141`):
...
- `error-classification.test.ts` — pattern table: each fault kind, timeout-flag precedence over aborted, aborted-neutral, unknown-string → non-provider, SDK subtypes → non-provider, auth-rebuild patterns → `auth` (no short-circuit) — **assert every alternate in `isAuthRebuildResumeError`'s regex individually classifies `auth`** (`resolve authentication`, `credentials.json`, `not authenticated`, `401 Unauthorized`, `ANTHROPIC_API_KEY`, `authToken`), so the auth row's superset property is regression-tested, not just asserted in prose; `classifyThrown` default.
...
- `agent-runner.test.ts` (extend) — deadline fire sets `timedOut: true` + `aborted: true`; operator abort sets `aborted` only; operator-abort-then-late-deadline (abort nulls `activeQuery` first, deadline fires after) leaves `timedOut` unset — the assertion the guard exists for.
