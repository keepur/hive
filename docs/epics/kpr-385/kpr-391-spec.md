...
- **Hard constraint — behavior-preserving:** every error-message string, sessionId fallback, self-heal trigger, abort checkpoint placement, deadline semantic (`error_turn_deadline`, `timedOut:true`+`aborted:false`, operator-abort priority — #407), and `RunResult` field is load-bearing (breaker classification regexes, `isStaleServerHandleError` sentinel, session persistence). The primary verification is that the full existing suites — 122 adapter tests (50 codex / 41 gemini / 31 openai) + 223 manager tests, vitest runtime counts at head `99e989e` — pass **without editing test expectations** (import/wiring changes only).
...
- **Deadline ownership (#407):** the scaffold owns the wall-clock deadline — arming inside the try, `clearTimeout` in the finally, the `deadlineFired` flag, the `error_turn_deadline` result shape, and the operator-abort-outranks-deadline interruption resolver (all three adapters' implementations are shape-identical here). The shared loop's abort checkpoints resolve interruption through that scaffold resolver (catching mid-tool expiry for codex/gemini); openai's SDK-loop quiet-resolve guard — a post-run `deadlineFired` check so an aborted SDK run that resolves quietly never surfaces partial text as success — stays in its own executeTurn, using the scaffold-exposed predicate.
...
| Cloned machinery | codex (741 ln) | gemini (770 ln) | openai (385 ln) |
|---|---|---|---|
| `abort()` / `wasAborted` / `currentAbortController` | 442–449 | 439–446 | 213–220 |
| `isAbortError()` (byte-identical ×3) | 455–460 | 517–522 | 294–299 |
| ToolBridge construction block (11 identical option lines; comments literally cite each other's line numbers) | 145–155 | 176–186 | 86–96 |
| `buildResult()` (~50–67 ln; codex ≡ gemini, openai = token-less variant; #407 added the `timedOut` flag ×3) | 462–516 | 524–578 | 301–350 |
| catch → `isAbortError` → interruption-vs-error result; finally → `clearTimeout` + `bridge.close()` + controller clear | 415–440 | 412–437 | 179–211 |
| `abortedResult()` closure carrying accumulated totals + bridge stats | 168–180 | 190–202 | — |
| Wall-clock deadline machinery (#407, ~60 ln each): `timeoutMs`/`deadlineFired` state, `deadlineResult()` (`error_turn_deadline`, `timedOut:true`+`aborted:false`), arming inside the try, `clearTimeout` in the finally; codex/gemini add an identical `interruptedResult()` operator-abort-priority resolver + deadline-aware stream-consumer `isAborted`; openai inlines the priority check + quiet-resolve guards | 123–139, 182–223, 434 | 155–170, 204–245, 431 | 47–81, 98–109, 153, 169, 184, 205 |
| Bounded dispatch loop skeleton: `maxRounds = resourceLimits?.maxTurns ?? 10`, round counter, `error_max_turns` result, abort checkpoints (post-#407 resolving via `interruptedResult`), per-round stream consume → totals accumulation, harvest, sequential tool execution, chain next input | 266–389 | 289–394 | (Agents SDK loop) |
| `executeFunctionCall` containment (unknown tool / bad JSON args → text, never throw) | 58–71 | 729–737 | (SDK `errorFunction`) |
| `errorMessage` / `objectField` / `stringField` helpers | ×3 / ×2 / ×2 | | |

Total: roughly **580 of the 1,896 adapter lines are clones** — and the gemini adapter's own header comment says it: its loop was built "on the codex-loop template" (KPR-352), its bridge block is "the codex adapter's exact construction".

The cost is not the lines; it's the divergence. Live examples, both from the same week:

- PR #407 (`66141da`, 2026-08-24) added the wall-clock turn deadline — hand-landed **three times**, ~60 near-identical lines per adapter, comments citing each other ("codex-adapter-identical shape", "codex/gemini-identical shape"), plus a two-round review whose fixes (arm-inside-try throw-safety, deadline-aware stream `isAborted`, `clearTimeout` pin) each had to be applied ×3. That is this ticket's cost model demonstrated on live history: one shared-machinery change = 3× the diff, 3× the review surface, 3× the drift risk.
- PR #405 (`2423f1f`, 2026-08-24) fixed status preservation on **stream-phase** errors — in gemini only (`describeStreamError`). Codex's stream-phase `response.failed` path (`codex-subscription-adapter.ts:639–643`, unchanged by #407) still throws message-only with no status prefix, so a mid-stream 429/5xx on codex can still reach the classifier bare — the exact bug class just fixed on its sibling.

Every new provider (KPR-392's grok is next) starts as a fourth clone of whichever sibling looks closest and inherits whichever fixes that sibling happened to get.
...
- **Abort lifecycle:** `currentAbortController` / `aborted` fields (declared at codex 109–110 / gemini 135–136 / openai 33–34), `abort()`, `wasAborted`, `isAbortError()` — today's byte-identical triplicate.
...
- **Wall-clock deadline (#407):** the scaffold arms the `resourceLimits.timeoutMs` timer *inside* the try (throw-safety pin), owns `deadlineFired`, fires the shared abort controller on expiry (warn log included), and clears the timer in the finally (no-late-warn pin). It owns the two result shapes and their priority: `deadlineResult()` (`error: TURN_DEADLINE_SUBTYPE`, `timedOut: true`, `aborted: false` — deliberately mismatching the Claude-lane `timedOut && aborted` hang rule) and the interruption resolver (`deadlineFired && !aborted ? deadline : aborted` — operator abort outranks a deadline that also fired). This ownership is behavior-preserving because the three #407 implementations are shape-identical on every one of these points; the only openai-specific piece — the quiet-resolve guard after an SDK run that absorbs its own abort — stays in openai's executeTurn, reading a scaffold-exposed `deadlineFired()` predicate. Deadline-path sessionId follows each adapter's existing catch-error shape (codex/gemini bare fallback — pinned by gemini's "never a mid-turn mint" test), which the fallback-session hook already parameterizes. The scaffold-owned `isAborted()` handed to provider code is `aborted || signal.aborted`, which subsumes #407's deadline-aware stream-consumer callbacks.
- **ToolBridge construct + close:** builds the bridge from `assembly` + request context (today's 11-line identical block, including the KPR-354 `delegateRunner` line); `close()` in the finally, always, including pre-request throw paths (pinned today by gemini T10, `gemini-interactions-adapter.test.ts:725–743`).
- **Containment frame:** the try/catch/finally — catch → `isAbortError` → interruption resolver (deadline-vs-abort, per the #407 bullet above); else error result via `errorMessage()`; finally → deadline `clearTimeout` + bridge close + controller clear. Provider code inside the frame *returns or throws*; nothing it throws escapes `runTurn`.
...
The provider subclass implements one abstract method, ~"executeTurn(harness)": receives `{request, bridge, signal, isAborted(), totals, …}`, performs auth resolution, `bridge.connect()`, and its dispatch, and returns a `TurnOutcome` (`{text, sessionId, error?}` — `error_max_turns` and friends are outcome errors, not throws, preserving accumulated totals). **Totals ownership (one writer):** provider round code reports per-round usage; the loop (or, for openai, the executeTurn body) folds it into the scaffold-owned accumulator — provider code never mutates the accumulator directly, so §4.2's "loop accumulates totals" and this section's scaffold-owned accumulator are the same mechanism, not two counters. **Ordering inside executeTurn stays provider-owned**: codex's history-load-before-auth ordering (KPR-353 T4-pinned deterministic degradation) and openai/gemini's auth-before-connect fast-fail are preserved verbatim because the scaffold does not sequence them.
...
- Abort checkpoints at today's exact placements: pre-round, post-stream-consume, pre-each-tool-execution, post-loop. Post-#407 each checkpoint resolves through the scaffold's interruption resolver, not a bare aborted result — this is where a deadline that expired inside a non-cancellable tool call is caught (the "between-checkpoint" pin in the codex/gemini deadline tests).
...
| Deadline expiry catch (#407) | scaffold arms/resolves; loop checkpoints catch mid-tool expiry | scaffold arms/resolves; in-executeTurn quiet-resolve guard (`deadlineFired()` predicate) | scaffold arms/resolves; loop checkpoints catch mid-tool expiry |
...
1. **Introduce scaffold + loop + module types** with their own unit tests (no adapter touched yet). The scaffold carries the #407 deadline machinery from birth (arming/cleanup/priority/result shape) — the deadline extraction rides this step, not a separate one.
2. **Migrate codex** (richest surface: history, heal, SSE) onto scaffold + loop + SSE util. Existing 50 codex tests (vitest runtime count) — including the six #407 deadline pins — run unedited (import paths at most). Exported test surfaces (`consumeCodexSse`, `consumeBufferedSseEvents`, option interfaces) keep their names and homes via re-export if files split.
3. **Migrate gemini** onto scaffold + loop. 41 tests unedited (incl. the four #407 deadline pins); `harvestFunctionCalls`, `applyInteractionEvent`, `__resetCoercionWarnedForTests` keep their exports.
4. **Migrate openai** onto scaffold only. 31 tests unedited (incl. the four #407 deadline pins — the quiet-resolve guard stays in its executeTurn).
...
11. **Wall-clock deadline (#407) — every pin, verbatim:** the `TURN_DEADLINE_SUBTYPE` string `"error_turn_deadline"`; expiry result shape `timedOut: true` + `aborted: false` (so the Claude-lane `timedOut && aborted` hang rule can never match); classification short-circuits to the dedicated `turn-deadline` kind, which the breaker treats as **inconclusive** — never trips, never resets a hard-fault streak, and a deadline-expired half-open probe reopens without backoff escalation, never a recovery close; the dispatcher's outage gate routes it down the legacy error path (`turn-deadline` ∉ `HARD_FAULT_KINDS` ⇒ no outage-queue replay of partially-executed tool turns); operator `abort()` outranks a deadline that also fired (breaker-neutral aborted result, no error surface); timer armed inside the try (a throw between arm and finally never leaks it) and cleared on normal completion (no late warn); `timeoutMs: 0` fires immediately but may still dispatch one round before aborting (timer = macrotask — distinct from `maxTurns: 0`'s zero-call short-circuit); `timeoutMs: undefined` arms no deadline (bare/test constructions); no history persist on a codex deadline turn; deadline-path sessionId = each adapter's catch-error shape (gemini's "never a mid-turn mint" pin); the deadline abort-chains into the ToolBridge and nested KPR-354 delegates via the shared signal, and the manager maps a **nested** delegate's deadline sentinel to prose Task-tool text ("the delegate exceeded its wall-clock deadline") with the nested 600s `timeoutMs` backstop staying live; openai's quiet-resolve guard (post-deadline SDK resolution never surfaces partial text as success).
- **Non-success sessionId policy (three-way, preserved exactly):** codex aborted and max-turns results use `lastResponseId ?? fallback` while its catch-error and deadline results use the bare fallback; gemini uses the bare fallback on all non-success paths despite tracking `lastInteractionId`; openai has no last-id tracking. The scaffold owns all these result paths after extraction, so this asymmetry is an explicit parameterization, not an accidental unification target.

## 8. Verification strategy

- **Primary pin — existing suites unedited:** 50 codex + 41 gemini + 31 openai + 223 manager + 29 breaker + error-classification (59) + turn-assembly (23) + tool-bridge (45) tests all pass with no expectation edits. (Counts are vitest runtime output at head `99e989e` — `it.each` expansion makes static `it(` greps differ; verify against vitest output, not grep. The deltas vs. the pre-#407 draft — +6 codex (44→50) / +4 gemini (37→41) / +4 openai (27→31) / +2 classification (57→59) / +3 breaker (26→29) — are exactly PR #407's deadline pins, all of which now also pin the extracted scaffold/loop.) This is the behavior-preservation proof; the suites already pin every §7 item (they were written as pins by KPR-350–354).
- **New tests (additive):**
  - Scaffold: bridge closed on success/error/abort/pre-request-throw paths; abort containment; `buildResult` clamp + toolSummary; fallback-session hook behavior per policy; deadline machinery (arm-inside-try throw-safety, clearTimeout on every exit path, `timeoutMs` 0/undefined semantics, operator-abort-outranks-deadline resolution, `error_turn_deadline` result shape).
