...
- **`id` is the model prefix** agents use (`sol/sol-large-2:high`). Validation at parse time: shape regex; **reserved-id rejection** — the full set `resolveProviderModel` recognizes plus aliases and the compatibility key: `claude`, `openai`, `openai-codex`, `codex`, `gemini`, `google-gemini`, `grok`, `kimi`, `deepseek`, `laneB` (and its lowercased near-miss `laneb`, unreachable via the lowercase-only id regex but rejected anyway to close the gap); **cross-plugin collision** — second registrant of the same id is rejected (declared-broken, §4.3) with both plugin names logged.
