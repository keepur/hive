...
- **Enumerated behavior-bearing deltas (the complete list — spec §7):**
  ...
  8. `src/agents/agent-runner.test.ts` — 4 compatibility literals (`buildToolTransportInventory` pins at lines 1178, 1374, 1429, 1454) gain a `grok:` key mirroring the codex value in each record (Task 1). Fixture-only; baseline 179 @ `e0236a9`, expected post-change count unchanged at 179. [Amended by driver adjudication 2026-08-26 — caught-by: implement/task1/sonnet; the pre-amendment reviewer sweep grepped `toHaveLength(3)`/id-triples, which misses `toEqual` record literals.]
