...
This artifact follows `docs/epics/kpr-451/` and the conventions in `CLAUDE.md` / `AGENTS.md`. Drafting does not approve an implementation plan. KPR-454's signed design body is not edited: overlapping-commit order was unspecified there, and a human-signed artifact is corrected by append-only supersession, never by editing the signed body (KPR-458 canon). This spec is that supersession for the reload-commit rule. KPR-454 D9's loaded-subscription-set paragraph is unchanged; only its unspecified overlapping-commit order is supplied here.
...
| `CLAUDE.md` **Ops tool-failure publishing (KPR-454)** bullet | One sentence on ordered publisher reloads (AC8), next to "the subscription set, by contrast, reloads every 60 s and on SIGUSR1". That ordered-commit wording is not in `CLAUDE.md` today — it lives in the KPR-468 canon bullet and in `notifier.ts` — so do not invent or "preserve" a notifier clause there. The KPR-468 **Ops notifier** bullet is untouched. |
...
7. **`stopping` skip still supersedes nothing.** After `stop()`, a `reloadSubscriptions()` call returns without a find (no new `operations` entry for `ops_subscriptions`/`find`) and without incrementing `subscriptionReloadFaults`. Existing enqueue-side `stopping` refusal and bounded drain are unchanged. (`stop()` does not clear `stopping`; a new process or a new `OpsPublisher` instance is the restart path, as today.) Do not add a test hook or `getSnapshot()` field to read `reloadsStarted`; D1 keeps that counter private, so the assertion that can fail is the absent find.
8. **`CLAUDE.md`.** The KPR-454 **Ops tool-failure publishing** bullet gains one sentence, next to "the subscription set, by contrast, reloads every 60 s and on SIGUSR1": publisher subscription reloads are ordered by start; an older completion cannot replace a later successful commit; failed or `stopping`-skipped reloads supersede nothing. It must not claim the notifier fix repaired the publisher, must not imply the two caches are shared, and must not invent a matching notifier sentence in `CLAUDE.md` (that wording is not there today).
...
- **Integration: required.**
  ...
  - Minimum assertions: AC1–AC7 as named tests. AC1 and AC2 each assert the **inserted document's** `matchedSubscriptionIds` and `matchedSubscriptions`, not merely `getSnapshot().subscriptions`. AC3–AC5 assert both the committed snapshot and the next event's stamp. AC6 asserts gauge, fault-counter and warn counts. AC7 asserts the absence of a find (not a `reloadsStarted` read — do not add a test hook for that counter). The disable/enable pair uses `pause(OPS_SUBSCRIPTIONS_COLLECTION, "find", () => true, true)` — `after=true` is load-bearing; `after=false` holds *before* the copy and does not reproduce the captured-stale-result race. Fault injection uses `failNext` / an nth-call `when` against that same `find`, never `stop()` as a mid-scenario barrier (`publisher.ts:419-420`: `stop()` sets `stopping`, so later enqueues silently no-op).
  - AC8 is a documentation assertion: the ordered-commit sentence is present in the KPR-454 **Ops tool-failure publishing** bullet of `CLAUDE.md`, adjacent to the existing 60 s / SIGUSR1 clause; a source scan or a focused string test is enough. Do not parse English beyond presence of the ordered-commit claim in that bullet and absence of a "notifier repaired the publisher" claim. Do not require (or add) the same sentence on the KPR-468 **Ops notifier** bullet — it is not there today.
