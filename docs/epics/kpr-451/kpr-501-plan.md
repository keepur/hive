...
- `src/ops/block-reasons.ts` — `HIVE_AGENT_PRODUCER`, the three reason-id constants, `BLOCK_SUBJECT_KIND`, `BLOCK_EVIDENCE_KIND_WORK_ITEM`, `BLOCK_EVIDENCE_KIND_RULING`, and `HIVE_AGENT_REASONS` in D3 order. Imports only `types.ts` / `ids.ts` (the `reasons.ts` layering) (Task 2).
- `src/ops/block-reasons.test.ts` — AC12 unit cases (Task 2).
- `src/ops/block-producer.ts` — pure layer (Task 4) and `buildBlockTools(deps)` handlers (Task 5).
...
- `src/ops/single-events-writer.test.ts` (new) — shaped like `src/ops/single-prefix-predicate.test.ts`: a repo-root derived from `import.meta.url`, a `tinyglobby` `globSync` over `src/**/*.ts`, comment lines stripped before matching, an expected-location set per needle.
- Current occurrences at the baseline (verified at plan time): `OPS_EVENTS_COLLECTION` in `src/ops/types.ts` (declaration) and `src/ops/store.ts` (import + collection handle); the literal `"ops_events"` only in `src/ops/types.ts`; `store.events.insertOne(` only in `src/ops/publisher.ts`'s accept path — plus a doc-comment quote in `src/ops/types.ts` that comment stripping must remove. Excluded: `*.test.ts` and `src/ops/testing/**` (`notifier-harness.ts` inserts directly; `lane-harness.ts` reads the constant). Unrelated bare `.events.insertOne(` hits exist in `src/workflow/event-emitter.ts` and `src/events/event-bus-mcp-server.ts`, which is why the needle is keyed on the `store.events.` handle.

**Dependencies:** None.

**Interfaces, compatibility, and invariants:**
- Three needles, three expected file sets: `OPS_EVENTS_COLLECTION` ⊆ {`src/ops/types.ts`, `src/ops/store.ts`}; `"ops_events"` (either quote style) ⊆ {`src/ops/types.ts`}; `store.events.insertOne(` ⊆ {`src/ops/publisher.ts`}. A hit elsewhere fails with the file and needle named.
...
- The `hive-agent` literal appears only in `block-reasons.ts` (and later `block-producer.ts`), never in a validator (KPR-454 AC16).
...
- Expected: all pass; the only changed existing assertion is the documented boot-case id set and call order.
...
- [ ] Add the unit tests and adjust the one boot/registry assertion with its reason in a comment.
...
**Acceptance criteria:** `enqueuePublish` returns before any insert; a `publish` job enqueued between two `enqueueFailure` jobs is stored between them in `(publishedAt, _id)` order; `openConditions` unchanged across `publish` jobs; `findOpenCondition` returns the condition's exact `dedupeKey` when open, `undefined` when never raised, `undefined` when a more recent `clearsFamily` event exists, the new key after re-report; a second `OpsPublisher` over the same `fakeDb.db` (restart) returns the same answer as the first; with `failNext("ops_events", "findOne")` armed on a drained queue, `findOpenCondition` rejects and `epochResolveFaults` is unchanged; `resolveGeneration` behaviour pinned by the existing suites; `flush` with a held insert (`FakeDb.pause(OPS_EVENTS_COLLECTION, "insertOne")`) and a small real deadline resolves not-idle, and after release resolves idle; `isReasonEnabled` reflects `ops_reasons` `enabled` after `init()` and is `false` for an unknown pair.
...
- Expected: new cases pass; every pre-existing `src/ops` test passes with no edit beyond Task 2's documented one.
...
- No `startsWith`/regex over a reserved prefix anywhere in the block modules; `single-prefix-predicate.test.ts` needs no allowlist change.
...
- Run: Unit command; `SLACK_APP_TOKEN=test SLACK_BOT_TOKEN=test SLACK_SIGNING_SECRET=test npx vitest run src/ops/single-prefix-predicate.test.ts src/ops/single-events-writer.test.ts`; `npm run typecheck`.
- Expected: all pass; NV7 and NV9(shape) observed failing then restored green.
...
- Expected: all pass; `src/agents/agent-runner.test.ts` and `src/events/event-bus-mcp-server.test.ts` unchanged and green; NV1–NV7 each observed failing then restored.
...
- Harness/environment: real `AgentRunner`s with the fixture's `fakeDb.db`; `buildLaneBHarness({ bridge })`; `buildOpsFixture()`; env stubs for `config.ts`.
...
**Acceptance criteria:** AC13 (the three documentation changes present; nothing claims delivery ships). AC14 (`git diff` against the task's base shows no change to `src/index.ts` or `src/boot-order.test.ts`; no `.start(` introduced in the diff). Full gate green; bundle guards green; `git diff --check` clean; spec hash unchanged.
...
- Run: `grep -n "report_block" CLAUDE.md` (expect the server-list line and the new bullet); `grep -n "Ops block producer (KPR-501" CLAUDE.md` (expect exactly one); `git diff --stat epic/kpr-451 -- src/index.ts src/boot-order.test.ts` (expect empty); `git diff epic/kpr-451 | grep -n "\.start("` (expect no added line); `SLACK_APP_TOKEN=test SLACK_BOT_TOKEN=test SLACK_SIGNING_SECRET=test npm run check`; `npm run check:bundle`; `git diff --check`; `shasum -a 256 docs/epics/kpr-451/kpr-501-design.md`.
...
2. **Task 2** — `feat(KPR-501): hive-agent reason table and table-list registration` (includes the one documented boot-case assertion adjustment).
...
- **Plan-level test placement.** Publisher-addition tests live in `block-producer.integration.test.ts` (so the "Integration (this child)" command covers them); the only edit to `publisher.integration.test.ts` is Task 2's documented boot-case adjustment.
