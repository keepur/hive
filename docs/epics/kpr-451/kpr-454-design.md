...
| --- | --- | --- | --- | --- | --- |
| `tool-failed` | `resource` | `transient` | — | `tool`, `errorSig`, `agentId?`, `lane`, `threadId?`, `workItemId?`, `durationMs?` | `"retry {tool}; if it repeats, inspect {errorSig} — last seen for agent {agentId} on lane {lane}"` |
...
**The map, and its single writer.** One process-global, capped `Map<family, { firstFailureAt }>` where `family = "hive-runtime:tool:<toolName>:tool-failed"`, evicted oldest-first at the cap. It lives on the publisher singleton (D9), not on a runner, because runners are per-turn and the condition is not.

**The map is mutated at accept time and only by the drainer.** Both sides — entry on an accepted failure insert, removal on an accepted `tool-recovered` insert — happen inside the single drainer, after the insert returns. The turn thread never writes it; the capture points only *read* it (`has`). This is what makes D9's "the drainer serializes the open-condition map" literally true rather than approximately true, and it is the reason the ownership is stated once, here, and nowhere else in this spec.
...
That cost is then removed outright: **the drainer re-checks `has(family)` before publishing a recovery job and drops the job if the family is already closed.** Because the drainer is serial and owns the map, the first queued recovery for a family accepts and removes the entry, and every duplicate behind it is a no-op counted as `recoveryCoalesced`. N successes in the drain window therefore produce exactly one `tool-recovered`, with no second structure and no turn-thread mutation. Accept-time removal is strictly better on both axes, which is why it is the one this spec pins.

**On success** (`PostToolUse`; Lane B's `:350`): a map read and nothing else. If the family is absent — the overwhelmingly common case — the observation returns immediately. **No database read on the success path**, per D2's explicit rejection of one against C15. If the family is present, one `tool-recovered` job is enqueued carrying `clears: <the open condition's dedupeKey>`; the entry is removed later, by the drainer, if and when that publish is accepted.
...
1. **AC1 (C1, C4)** — A published event carries no `owner`, `severity`, `destination`, `recipient` or rendered string, and `class`/`retry` equal the registry row's. The internal observe API has no parameter that could supply them; a test asserts the stored document's key set is exactly the D9 list — including `schemaVersion === 1` and a server-stamped `publishedAt` — with `clears`/`clearsFamily` present on a `tool-recovered` document and absent on a `tool-failed` one, and fails on any extra key.
...
3. **AC3 (C5)** — A disabled or unknown `(producer, reasonId)`, an undeclared `detail` key, an over-bound `detail` value, a non-scalar value and an over-length `subject.id` each reject the publish and increment the rejection counter. No coercion to a generic reason, and no truncation of `subject.id`.
...
- **⚠ Delegated (non-blocking) — the open-condition map is drainer-owned, mutated only at accept time.** Round-2 review found the draft asserting both accept-time and enqueue-time removal. Resolved to accept-time on failure modes (D8): enqueue-time removal can lose a clearing fact outright, accept-time removal cannot, and its only cost — duplicate recovery jobs in the drain window — is erased by the drainer's `has(family)` re-check. Cost accepted: entry is also accept-time, so a success landing inside a failure's enqueue window leaves the condition open one cycle longer. D8.
