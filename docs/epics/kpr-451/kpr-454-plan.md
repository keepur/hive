...
| `SHUTDOWN_DRAIN_MS` (D10) | `2000` | Shutdown already awaits Slack and Mongo; the ops queue must not add a visible stall to a `launchctl kickstart`. At the measured per-job cost, 2 s clears several hundred jobs — more than a healthy queue ever holds — and a queue deeper than that at shutdown means a storm was in progress, where the marginal rows are again restatements. On timeout the remainder is dropped and counted as `drainDropped`. ⚠ **Amended in review (`81d7c05`):** `drainDropped` is no longer the deadline drop alone. `enqueue()` also increments it once per observation refused *after* the `stopping` latch is set — that window is not instantaneous (`stop()` latches, then drains for up to `SHUTDOWN_DRAIN_MS` while turns are still running), so the arm was swallowing real failures with no counter. Both arms mean "shutdown lost N ops observations", they cannot overlap in time, and no reader distinguishes them — hence one field, not two. The shipped source is authoritative; see the amendment note at the top of `kpr-454-plan-3-publisher.md`. |
