...
- One structure ⇒ one source of truth. The runtime consumers — the eligibility filter (`:2302`) and `sweep()` — need at most a type-level change, and no future site can read "excluded" from the wrong structure. The **test harness is not type-level-only**: its `excludedFor` accessor (`dispatcher-conference.test.ts:267-274`) returns the tracker leaf directly, and four assertions compare that leaf **by value** against `new Set([...])` (`:1576`, `:1673`, `:1696`, `:2012`) — a `Map` never `toEqual`s a `Set`. The accessor therefore gains one small behavioral change, a **key projection**: return `new Set(leaf.keys())`, preserving `undefined` when the leaf is absent. That keeps the existing `responded`-presence meaning (membership = excluded, Key Points bullet 2), so all four value assertions and every `toBeUndefined()` read (`:1659`, `:1737`, `:2035`, `:2060`) stand byte-identical.
...
| Surface | Change |
|---|---|
| `dispatcher.ts:206` tracker decl + `:200-205` doc comment | leaf `Set<string>` → `Map<string, ReactionTrackerEntry>`; comment gains the provenance-tag sentence and drops "Set" phrasing — specifically, the closing sentence "Shape, keying and TTL are unchanged (C2)." (`:205`) is falsified by the leaf change and must be rewritten (keying/TTL unchanged; leaf shape amended by KPR-420 to carry provenance) |
| `dispatcher.ts:689-703` `markReactionExclusion` + doc (`:685-686`) | `.add` → `.set("delivery-mark")` (`:701`); the doc sentence "Synchronous and idempotent (Set add). Tracker shape, keying and TTL are unchanged — KPR-386 canon C2 preserved, C1 superseded." (`:685-686`) is falsified on both halves ("Set add" and "shape … unchanged") and must be rewritten to name the Map leaf + provenance amendment |
...
| `dispatcher-conference.test.ts:267-274` `excludedFor` (incl. the `:270` harness type) | harness type updated to the Map leaf **and** the accessor gains a key projection — return `new Set(leaf.keys())`, `undefined` preserved when the leaf is absent — so the four value-level `toEqual(new Set([...]))` assertions (`:1576`, `:1673`, `:1696`, `:2012`) pass unmodified with membership meaning unchanged (§4, §9 T3) |
...
| root `CLAUDE.md` (Meeting mode section) | per C9, docs-sync on this child branch: amend the "Accepted residual (deliberately deferred by KPR-416)" sentence — the release loop no longer erases delivered marks (KPR-420); residual narrows to claim-time only — **and** correct the now-falsified clause "preserves C2's tracker shape/keying/TTL" in the same bullet (keying/TTL preserved; leaf shape amended to the provenance Map by KPR-420) |
...
- **T3 (existing suite).** `dispatcher-conference.test.ts` passes with the harness type update **plus one small behavioral change to the `excludedFor` accessor** (`:267-274`): it must project the Map leaf's keys to a value-level set — return `new Set(leaf.keys())`, preserving `undefined` when the leaf is absent — because four existing assertions compare the leaf by value (`toEqual(new Set(["jasper"]))` at `:1576`, `:1673`, `:1696`, `:2012`) and a `Map` never `toEqual`s a `Set`. "Type-level only" is false against the current file; the accessor's key projection is the whole harness delta, and **no assertion is rewritten** — the projection preserves the `responded`-presence meaning (Key Points bullet 2), so the four value assertions and the `toBeUndefined()` reads pass unmodified. In particular the `:517` exclusion guard, T1/T6/T8a/T8b (KPR-416), and T9 stay unchanged in behavior. The `:531` macrotask suppression **stays** with a one-line comment pointing at T1 as the unsuppressed sibling.
