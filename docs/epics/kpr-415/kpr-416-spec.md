...
- **Regression risk 2 — KPR-388 delta injection:** re-derived in §7.2 with a **new, stronger invariant** replacing the one the in-code comment at `dispatcher.ts:1344-1351` asserts (load-bearing sentence at `:1347-1349`). That comment must be rewritten, not deleted; replacement text is given in §7.2. Note grok — this epic's poster child — never reaches the affected path at all (stateless-replay providers have no `sessionId`, so they always take the full/summary arm, `:1723`).
...
|---|---|---|---|
| Post-turn outage queue (breaker open) | `:1404` early return | excluded | **not excluded** (queued; the replay's own delivery marks it — §6.2) |
...
Chain behavior with the chosen fix: the key survives every leg (`...carriedMeta`); the first leg that delivers text writes the exclusion; a chain that exhausts its cap writes nothing (correct — nothing was answered); a leg that itself throws writes via `handleTurnFailure` (correct — it posted an error). The epic design's noted sub-residual — a leg re-queues under `x#dl<n>` so `statusOf` on the origin id can't see it — is dissolved by this choice rather than inherited, because nothing in this child queries `statusOf` (see 6.4).
...
1. **The per-thread lock serializes the in-flight half.** `withSpawnTicket` keys on `` `${ctx.agentId}:${ctx.threadId}` `` and spin-waits while held (`agent-manager.ts:1452-1459`). B's round-1 reaction turn therefore cannot run concurrently with B's own round-0 turn; it queues behind it. Outcome: B answers round-0, then reacts to A — in that order, with round-1's `reactionTo` framing explicitly forbidding a re-answer (`:1352-1356`) and B's own answer already in its session. That is an extra turn and extra cost, not a duplicate answer.
...
**Comment rewrite required.** Replace `dispatcher.ts:1344-1351`'s third-and-fourth sentence (the C1/C2 claim) with the updated invariant above, citing KPR-416 and noting that C1 (selection-time recording) is superseded. Do not delete the surrounding KPR-387/KPR-388 framing — only the falsified premise.
...
- ⚠ **Assumption:** engine-authored notices are not agent content and must not mark exclusion. Stated as the rule in §4; if a future reviewer disagrees, the affected rows are the outage-queue, replay-failure and cap-exhausted rows of §4's table.
- ⚠ **Assumption:** `meetingExclusionTs` is a safe new key name — nothing else reads or filters `meta` by allowlist, and the only meta blocklist in the file (`:846-853`) names five keys explicitly. Verify at implementation time that no later ticket has added an allowlist.
...
- **T5 — ordering pin.** A concurrent peer's reaction pass cannot claim an agent as a reactor in the window between that agent's own round-0 delivery and its exclusion write; guards against the write drifting after `:1481`/`:1485`.
...
- **T8 — disposition (c), two parts.**
  ...
  - **T8b (behavior):** that continuation leg's delivery on the single-dispatch leg excludes the agent from reaction eligibility. Companion: a chain that exhausts `MAX_DEADLINE_CONTINUATIONS` (terminal notice, no answer) does **not** exclude.
...
- **Preserves KPR-388's covering invariant** verbatim; only which half of it carries the round-1 case changes (§7.2).
