...
This is the most error-prone section — shared-HIVE_HOME dodi+personal case, legacy-label rename (dodi only), health timeout.

Key design decision (see spec §Step 10): the engine does NOT ship plist templates. `hive daemon start` builds plists dynamically via `buildPlist()` in `src/cli/daemon.ts:27`, deriving the label from `instance.id` in whichever config file `HIVE_CONFIG` points at. We regenerate **one plist per `hive.yaml` config file** present at the instance root — one `hive daemon start` invocation per yaml.

Dodi's legacy label `com.hive.agent` doesn't match the `com.hive.<id>.agent` convention `hive daemon start` produces. We retire it explicitly (bootout + delete symlink) before the regeneration loop. The rename is customer-visible; call out in 0.2.0 release notes.
...
## Rollout plan
- **Keepur is no longer a canary** — it's already on the 0.2.0-ish layout (verified 2026-04-22: `~/services/hive/keepur/` has no `dist/`, `node_modules/`, or `.git/`; live plist is `com.hive.keepur.agent.plist`). Preflight will classify it as "already migrated" and exit 0.
- **Scratch-instance canary first.** Stand up a disposable 0.1.x instance in `~/services/hive/scratch-0.1/` against a throwaway MongoDB db, run the migration end-to-end, verify auto-rollback works synthetically. De-risks dodi.
- **dodi after scratch canary green.** Low-activity window. Dodi is shared HIVE_HOME with personal — both go offline for the duration (~5 min). Legacy label `com.hive.agent` is retired; new labels are `com.hive.dodi.agent` + `com.hive.personal.agent`. Call out the dodi label rename in the 0.2.0 release notes.
- **Beta customer self-migration** one-liner published in `hive-docs` once dodi is stable for 7d.

## Test plan
- [ ] `install/migrate-0.2.test.sh` — dry-run harness green
- [ ] `bash -n` and `shellcheck` clean
- [ ] Real-mode test on the scratch instance: preflight → snapshot → skeleton → wipe → classify → populate → install engine deps → yaml rewrite → remove .git → retire legacy label → regenerate plists → health check → notify. All steps succeed, final `ls <instance>/` shows only the expected dirs.
- [ ] Idempotence: re-run against the migrated instance, exits "already on 0.2.0."
- [ ] Keepur idempotence: preflight against `~/services/hive/keepur/` exits 0 with "already migrated" without touching the instance.
- [ ] Auto-rollback: synthetic failure (e.g., HIVE_VERSION=0.0.0-broken), confirm `.pre-0.2-bak` restores a working instance.
- [ ] Plist regeneration: after migration, `launchctl print gui/$(id -u)/com.hive.<id>.agent` shows `ProgramArguments` ending in `.hive/pkg/server.min.js` for every config file at the instance root.
...
- Stand up scratch 0.1.x instance at `~/services/hive/scratch-0.1/`, run the migration end-to-end including synthetic rollback.
- Execute dodi migration in a low-activity window after scratch-canary green: `bash install/migrate-0.2.sh ~/services/hive/dodi`. Watch for 24h.
EOF
)"
```
...
- **Parallel per-instance migration** — script migrates one instance per invocation. Shared HIVE_HOME (dodi+personal) migrates both in the same invocation.
- **Automating the rollout** — scratch-canary-then-dodi sequencing is a human decision with Slack comms; don't script it.
