...
## Task 13: (Optional) `src/cli/migrate.ts` wrapper
...
Note: if this change happens, `deploy.sh` (KPR-53) `fetch_engine` and `src/setup/populate-engine.ts` (KPR-54) `PACKAGE_ENTRIES` also need `install/` appended. Coordinate or defer Task 13 entirely — that ripple is the reason this task is optional.
...
- [ ] **Step 5:** Commit (if Task 13 happens at all).
